const got = require('@/utils/got');
const timezone = require('@/utils/timezone');
const { parseDate } = require('@/utils/parse-date');
const { art } = require('@/utils/render');
const path = require('path');
const UA = require('@/utils/rand-user-agent')({ browser: 'mobile safari', os: 'ios', device: 'mobile' });

module.exports = async (ctx) => {
    const channel = ctx.params.channel ?? 1;
    const currentUrl = `https://www.xkb.com.cn/xkbapp/fundapi/article/api/articles?chnlId=${channel}&visibility=1&page=0&size=15&keyword=`;

    const { data: response } = await got({
        method: 'get',
        url: currentUrl,
        headers: {
            siteId: '35',
            'User-Agent': UA,
        },
    });

    const list = response.data
        .map((item) => ({
            title: item.listTitle,
            description: art(path.join(__dirname, 'templates/description.art'), {
                thumb: item.metaInfo.thumbnails[0],
            }),
            pubDate: timezone(parseDate(item.operTime), +8),
            link: 'https://www.xkb.com.cn/#/detail?id=' + item.id,
            contentUrl: item.contentUrl,
            author: item.metaInfo.author,
            chnlName: item.metaInfo.chnlName,
        }))
        .filter((i) => i.contentUrl); // Remove "专题报道" (special report)

    let chnlName = '';

    const items = await Promise.all(
        list.map((item) =>
            ctx.cache.tryGet(item.contentUrl, async () => {
                const detailResponse = await got({
                    method: 'get',
                    url: item.contentUrl,
                });
                const data = detailResponse.data.htmlContent.replace(/ \n/, '').replace(/\\/, '');

                item.description += data ?? '';
                chnlName = chnlName === '' ? item.chnlName : chnlName;
                return item;
            })
        )
    );

    ctx.state.data = {
        title: `新快报新快网 - ${chnlName}`,
        link: `https://www.xkb.com.cn/#/home?id=${channel}`,
        item: items,
    };
};
