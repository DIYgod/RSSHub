...
module.exports = async (ctx) => {
    ...
    const list = $(nwafuMap.get(type)[1])
        .map((index, ele) => {
            ...
            let itemLink = new URL($(ele).find(nwafuMap.get(type)[2]).attr('href'), nwafuMap.get(type)[0]).toString();
            for (const item of nwafuMap.get('forbiddenList')) {
                if (itemLink.search(item) !== -1) {
                    itemLink = nwafuMap.get(type)[0];
                    break;
                }
            }
            return {
