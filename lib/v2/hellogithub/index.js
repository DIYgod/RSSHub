...
module.exports = async (ctx) => {
    const category = ctx.params.category ?? 'hot';
    ...
    const currentUrl = `${rootUrl}/?sort_by=${category}${id ? `&tid=${id}` : ''}`;
    const apiUrl = `${apiRootUrl}/v1/?sort_by=${category}${id ? `&tid=${id}` : ''}&page=1`;
    ...
    ctx.state.data = {
        title: `HelloGithub - ${categories[category]}${tag || ''}项目`,
        link: currentUrl,
        item: items,
    };
};
