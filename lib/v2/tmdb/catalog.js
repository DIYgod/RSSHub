...
module.exports = async (ctx) => {
    const { mediaType, catalog, lang } = ctx.params;
    const { data } = await got(`https://api.themoviedb.org/3/${API_PATH[mediaType][catalog]}`, {
        searchParams: {
            language: lang,
            api_key: apiKey(),
        },
    ...
    ctx.state.data = {
        title: `${TITLE[mediaType][catalog]} — TMDB`,
        link: `https://www.themoviedb.org/${mediaType}/${catalog}`,
