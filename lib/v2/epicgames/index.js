...
module.exports = async (ctx) => {
    ...
    const items = response.data.data.Catalog.searchStore.elements
        ...
        .map((item) => ({
            ...
            link: `${rootUrl}/${locale}/p/${
                item.catalogNs.mappings.length > 0 ? item.catalogNs.mappings[0].pageSlug : item.offerMappings.length > 0 ? item.offerMappings[0].pageSlug : item.productSlug ? item.productSlug : item.urlSlug
            }`,
