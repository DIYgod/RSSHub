    ...
    const items = response.data.data.Catalog.searchStore.elements
        .filter(
            (item) =>
                ...
                new Date(item.promotions.promotionalOffers[0].promotionalOffers[0].startDate) <= now &&
                new Date(item.promotions.promotionalOffers[0].promotionalOffers[0].endDate) > now
        )
        .map((item) => ({
            title: item.title,
            author: item.seller.name,
            link: `${rootUrl}/en-US/p/${item.catalogNs.mappings.length > 0 ? item.catalogNs.mappings[0].pageSlug : item.offerMappings.length > 0 ? item.offerMappings[0].pageSlug : item.urlSlug}`,
