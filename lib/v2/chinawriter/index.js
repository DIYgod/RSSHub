        ...
        items.map((item) =>
            ctx.cache.tryGet(item.link, async () => {
                try {
                    ...
                    item.pubDate = content('div.end_info em').text() ? timezone(parseDate(content('div.end_info em').text(), 'YYYY年MM月DD日HH:mm'), +8) : parseDate(content('meta[name="description"]').prop('content'));
                } catch (e) {
