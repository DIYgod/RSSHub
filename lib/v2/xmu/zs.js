...
const host = 'https://zs.xmu.edu.cn';
module.exports = async (ctx) => {
    ...
    const typeName = $('.submenu-title').first().text() || '招生网';
    const list = $('.listcolumn-con .news-item');
    const items = await Promise.all(
        Array.from(list).map((item) => {
            item = $(item);
            const itemDate = item.find('.news-item-date').text();
    ...
    ctx.state.data = {
        title: `厦门大学招生网 - ${typeName}`,
        link: pageUrl,
        description: `厦门大学招生网 - ${typeName}`,
        item: items,
    };
};
