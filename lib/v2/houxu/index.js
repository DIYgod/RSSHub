...
module.exports = async (ctx) => {
    ...
    const currentUrl = `${rootUrl}/api/1/${id ? `lives/${id}/threads` : category === 'events' ? category : category === 'updated' || category === 'memory' ? 'lives/updated' : `records/${category}`}/?limit=${ctx.query.limit ?? 100}`;
            ...
            items.map((item) =>
                ctx.cache.tryGet(item.guid, async () => {
                    try {
                        const detailResponse = await got({
                            method: 'get',
                            url: `${rootUrl}/api/1/lives/${item.guid}/threads/?limit=100`,
                        });
                        ...
                        const feeds = detailResponse.data.results.map((item) =>
                            art(path.join(__dirname, 'templates/description.art'), {
                                ...
                                description: `<p>${item.link?.description}<try/p>`,
