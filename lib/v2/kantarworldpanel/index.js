...
module.exports = async (ctx) => {
    ...
    const rootUrlRegExp = new RegExp(rootUrl.replace(/(\.|\/)/g, '\\$1'));
    ...
    let items = $('ul.slide, #newssource')
        ...
        .map((item) => {
            ...
            return {
                ...
                guid: rootUrlRegExp.test(item.link) ? `${link}#${title}` : link,
    ...
    items = await Promise.all(
        items.map((item) =>
            ctx.cache.tryGet(item.link, async () => {
                ...
                if (item.link === currentUrl || !rootUrlRegExp.test(item.link)) {
                    return item;
