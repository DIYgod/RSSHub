...
module.exports = async (ctx) => {
    ...
    ctx.state.data = {
        ...
        item: data.map((item) => ({
            title: '观测时间：' + timezone(responseData.updateTime, +8),
            description: art(path.join(__dirname, './util/now.art'), { item }),
            pubDate: timezone(responseData.updateTime, +8),
            guid: '位置:' + ctx.params.location + '--时间：' + timezone(responseData.updateTime, +8),
            link: item.fxLink,
        })),
