...
module.exports = async (ctx) => {
    ...
    ctx.state.data = {
        ...
        item: data.map((item) => ({
            title: '观测时间：' + timezone(parseDate(responseData.updateTime, 'YYYY年MM月DD日 HH:mm'), +8),
            description: art(path.join(__dirname, './util/now.art'), { item }),
            pubDate: timezone(parseDate(responseData.updateTime, 'YYYY年MM月DD日 HH:mm'), +8),
            guid: '位置:' + ctx.params.location + '--时间：' + timezone(parseDate(responseData.updateTime, 'YYYY年MM月DD日 HH:mm'), +8),
            link: item.fxLink,
        })),
