...
const { getUserInfo, getUserFeedItems, getTagsFeedItems } = require('./utils');
...
module.exports = async (ctx) => {
    ...
    if (cacheMiss) {
        ...
        for await (const c of cookie.split('; ')) {
            await cookieJar.setCookie(c, 'https://instagram.com');
        }
    } else {
    ...
    switch (category) {
        ...
        case 'tags': {
            ...
            items = await getTagsFeedItems(tag, 'recent', cookie, cookieJar, ctx.cache.tryGet);
            // items = await getTagsFeedItems(tag, 'recent', cookie, ctx.cache.tryGet);
            break;
        }
        default:
            break;
