...
module.exports = async (ctx) => {
    ...
    const response = await gotByPPT(result.link);
    ...
    const actList = await Promise.all(
        [...Array(pageCnt).keys()].map(async (pageIdx) => {
            const queryParams = {
                page: pageIdx + 1,
                type,
            };
            ...
            const response = await gotByPPT(url);
            const $ = cheerio.load(response);
    ...
    ).then((actPageList) => actPageList.reduce((actPage, actPageAggr) => actPageAggr.concat(actPage), []));
    ...
    const actListFull = await Promise.all(
        actList.map((item) =>
            ctx.cache.tryGet(item.link, async () => {
                const response = await gotByPPT(item.link);
                const $ = cheerio.load(response);
