const got = require('@/utils/got');
const cheerio = require('cheerio');
const timezone = require('@/utils/timezone');
const { parseDate } = require('@/utils/parse-date');
const dateParser = require('@/utils/dateParser');

module.exports = async (ctx) => {
    const baseUrl = 'https://www.parliament.go.th/section77';
    const { type = '' } = ctx.params;

    let title = 'ร่างพระราชบัญญัติที่เปิดรับฟังความคิดเห็นตามมาตรา 77 ของรัฐธรรมนูญ';

    if (type) {
        const [ presenter, isMonetaryAct = '' ] = type.split('-');

        title += {
            presentbymp: 'ที่เสนอโดยสมาชิกสภาผู้แทนราษฏร',
            presentbymp: 'ที่เสนอโดยประชาชน',
            openwsu: 'ที่กำลังเปิดรับฟังความคิดเห็น',
            closewsu: 'ที่ปิดรับฟังความคิดเห็น',
            reportwsu: ' รายงานผลการรับฟังความคิดเห็น',
            substatus1: 'ซึ่งรอคำรับรองจากนายกรัฐมนตรี',
            substatus2: 'ที่บรรจุเข้าระเบียบวาระ',
            substatus3: 'ที่พิจารณาแล้ว',
            closewsubypm: 'ที่นายกฯ ไม่รับรอง',
        }[presenter] ?? '';

        title += {
            m: ' (ประธานสภาผู้แทนราษฎรวินิจฉัยว่า เป็นร่างการเงิน)',
            nm: ' (ประธานสภาผู้แทนราษฎรวินิจฉัยว่า ไม่เป็นร่างการเงิน)',
        }[isMonetaryAct] ?? '';
    }

    const result = {
        title,
        link: `${baseUrl}/survey_more_news.php?type=${type}`,
        language: 'th-th',
        item: [],
    };

    // Fetch the news first so we can know how many pages do we need to fetch
    const { data: response } = await got(result.link);
    const $ = cheerio.load(response);
    const maxPageElem = $("#pagination-more_new a[id]:last-child");

    if (!maxPageElem.length) {
        return result;
    }

    const pageCnt = parseInt(maxPageElem.attr('id'));
    const actList = await Promise.all(
        [...Array(pageCnt).keys()].map(async (page) => {
            const queryParams = {
                page,
                type
            };
            const url = `${baseUrl}/ajax/pagination.php?${new URLSearchParams(queryParams).toString()}`;

            const { data: response } = await got(url);
            const $ = cheerio.load(response);

            if ($.text() === 'ยังไม่มีข้อมูล') {
                return [];
            }

            return $('div.item-77')
                .toArray()
                .map((item) => {
                    item = $(item);
                    return {
                        title: item.find('a').text(),
                        link: `${baseUrl}/${item.find('a').attr('href')}`,
                        category: item.find('label').toArray().map(l => l.text()),
                    };
                });
        })
    ).then((actPageList) =>
        actPageList.reduce(
            (actPage, actPageAggr) => actPageAggr.concat(actPage),
            []
        )
    );

    if (!actList.length) {
        return result;
    }

    const actListFull = await Promise.all(
        actList.map((item) =>
            ctx.cache.tryGet(item.link, async () => {
                const { data: response } = await got(item.link);
                const $ = cheerio.load(response);

                // Select the first element with the class name 'comment-body'
                item.title = $('.title h5').first().text();
                item.author = $('.present-by').first().text();
                item.description = $('.des').first().html();

                // Act draft status
                const [, presenter, monetaryType] = $('.type77 h5').text().split(' ');
                item.category.concat(
                    $('.container-fluid .bg-status .col-md-8.p-0 h5 span,a')
                        .toArray()
                        .map(statusElem => statusElem.text())
                );
                item.category.push(presenter);
                item.category.push(monetaryType);

                const voteText = $('.row.bg-status .col-md-4.text-right').text().trim();
                const voteRegex = /^ผู้แสดงความคิดเห็น\s*(\d+)\s*คน\s*(\d+(?:\.\d+)?)%\s*(\d+(?:\.\d+)?)%/g.exec(voteText);

                if (voteRegex) {
                    const voteTotal = parseInt(voteRegex[0]);
                    const upvotePercent = parseFloat(voteRegex[1]);
                    const downvotePercent = parseFloat(voteRegex[2]);

                    item.upvotes = parseInt(upvotePercent / 100 * voteTotal);
                    item.downvotes = parseInt(downvotePercent / 100 * voteTotal);
                }

                const dateText = $('.banner-detail .banner-detail-caption .blockquote p:last-child')
                    .text();
                const dateRegex = /^รับฟังตั้งแต่วันที่\s(\d{1,2})\s*(\S+)\s*(\d{4})/g.exec(dateText);

                if (dateRegex) {
                    item.pubDate = dateParser(
                        `${dateRegex[0]} ${dateRegex[1]} ${dateRegex[2]}`,
                        'D-MMMM-YYYY',
                        'th',
                        +7
                    )
                }

                // Every property of a list item defined above is reused here
                // and we add a new property 'description'
                return item;
            })
        )
    );

    if (!actListFull.length) {
        return result;
    }

    result.item = actListFull;

    ctx.state.data = result;
};
