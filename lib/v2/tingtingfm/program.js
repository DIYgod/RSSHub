...
module.exports = async (ctx) => {
    ...
    const radioInfo = await ctx.cache.tryGet(`tingtingfm:program:${programId}`, async () => {
        const { data: response } = await got.post('https://api-v3.tingtingfm.com//broadcast/get_program_v3_8', {
            searchParams: {
                ...params,
    ...
    const latestAudio = await ctx.cache.tryGet(
        ...
        async () => {
            const { data: response } = await got.post('https://api-v3.tingtingfm.com//broadcast/get_program_audio_list', {
                searchParams: {
                    ...params,
                    api_sign: sign(params),
                },
    ...
    const audioList = await ctx.cache.tryGet(
        ...
        async () => {
            ...
            const { data: response } = await got.post('https://api-v3.tingtingfm.com//albumaudio/play_audio', {
                searchParams: {
                    ...playAudioParams,
    ...
    const items = list.map((audio) => ({
        title: audio.title,
        link: `https://mobile.tingtingfm.com/v3/vod/2/${audio.h_audio_id}`,
    ...
    ctx.state.data = {
        ...
        link: `https://mobile.tingtingfm.com/v3/program/${programId}`,
