...
const getInfoContent = async (ctx, url) => {
    ...
    return await ctx.cache.tryGet(`govPublicInfo${infoId}`, async () => {
        ...
        const rawDate = $('#symbol > div:nth-child(1) > div:nth-child(3)')
            .text()
            .split('\n')[1]
            .replace(/(^\s*)|(\s*$)/g, '');
        return {
            title: $('#headline').text(),
            id: infoId,
            infoNum: $('#symbol > div:nth-child(1) > div:nth-child(2) > span')
                .text()
                .split('\n')[1]
                .replace(/(^\s*)|(\s*$)/g, ''),
