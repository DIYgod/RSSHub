...
const getInfoContent = async (ctx, url) => {
    ...
    const content = await ctx.cache.tryGet(`govPublicInfo${infoId}`, async () => {
        ...
        return {
            ...
            date: parseDate($('#symbol > div:nth-child(1) > div:nth-child(3)').text().split('\n')[1].replace(/(^\s*)|(\s*$)/g, "")),
            ...
            file: $("#symbol > div:nth-child(3) > div > a").attr('href') ? `${rootUrl}/${$("#symbol > div:nth-child(3) > div > a").attr('href')}` : null,
            link: url
        };
    });
    return content;
...
module.exports = async (ctx) => {
    ...
    const items = await Promise.all(infoUrlList.map(async (item) => await getInfoContent(ctx, infoUrlList[item])));

    ctx.state.data = {
        title: `政府信息公开 - ${countyName} ${institutionName}`,
