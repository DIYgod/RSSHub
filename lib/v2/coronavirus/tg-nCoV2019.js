...
module.exports = async (ctx) => {
	...
	const filter = ctx.params.filter==='1' ? true : false;
	...
	if (list.length === 0 && $('.tgme_channel_history').length === 0) {
        throw `Unable to fetch message feed from this channel. Please check this URL to see if you can view the message preview: https://t.me/s/nCoV2019`;
    }
        ...
        item:
            ...
            list
                .map((index, item) => {
					...
					if (filter && item.find('div.tgme_widget_message_text.js-message_text > a').text().match(/#全国|#CN/g)) {
						return;
					}
					return {
						title,
