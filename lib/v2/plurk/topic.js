...
module.exports = async (ctx) => {
    ...
    const { data: names } = await got.post(`${baseUrl}/Users/fetchFriends`, {
        form: {
    ...
    const items = Object.values(apiResponse).map((item) => getPlurk(`plurk:${item.plurk_id}`, item, names[item.user_id].display_name, ctx.cache.tryGet));
