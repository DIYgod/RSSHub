...
const { documentToHtmlString, processVideo } = require('./react-renderer');
...
const parseReactRendererPage = async (res, api, item) => {
    ...
    const story_id = _.get(JSON.parse(json), api.prop);
    res = await got(`${idUrl}${story_id}`, { headers });
    return await parseStoryJson(JSON.parse(res.data.trim()), item);
};
