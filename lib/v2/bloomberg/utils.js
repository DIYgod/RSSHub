...
const parseArticle = (item, ctx) =>
    ctx.cache.tryGet(item.link, async () => {
        ...
        if (group) {
            ...
            if (apiEndpoints[page] && page === 'newsletters') {
...
const parseNewsletterPage = async (res, api, item) => {
    ...
    fs.writeFileSync(__dirname + '/' + item.title, JSON.stringify(newsletter_json, null, 2));
    ...
    const rss_item = {
        ...
        description: processHeadline(story_json) + (await processLedeMedia(story_json)),
