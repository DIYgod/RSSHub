...
const parseSearchHit = async (ctx) => {
    const variables = {
        ...
        start: '0',
        geoUrn: location[ctx.query.location] || `urn:li:ks_geo:${ctx.query.geo || location.china}`,
...
const parseJobPosting = async (jobPosting) => {
    ...
    return {
        ...
        link: `https://www.linkedin.cn/incareer/jobs/view/${jobPosting.entityUrn.split(':').pop()}`,
        guid: `linkedincn:${jobPosting.entityUrn}`,
        description: art(path.join(__dirname, 'templates/posting.art'), job),
