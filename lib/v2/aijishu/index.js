...
module.exports = async (ctx) => {
    ...
    const items = await Promise.all(list.filter((item) => item?.url?.startsWith('/a/') || (item.objectType === 'article' && item?.object?.url.startsWith('/a/'))).map((item) => utils.parseArticle(item, ctx)));
