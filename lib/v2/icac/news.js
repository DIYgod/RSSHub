const got = require('@/utils/got');
const cheerio = require('cheerio');
const utils = require('./utils');
const { parseDate } = require('@/utils/parse-date');

const { BASE_URL } = utils;

module.exports = async (ctx) => {
    const res = await got.get('https://www.icac.org.hk/tc/press/index.html');
    const $ = cheerio.load(res.data);

    const list = $('.pressItem.clearfix')
        .map((_, e) => {
            const c = cheerio.load(e);

            return {
                link: `${BASE_URL}${c('.hd a').attr('href')}`,
            };
        })
        .get();
    const items = await Promise.all(
        list.map((item) =>
            ctx.cache.tryGet(item.link, async () => {
                const detailResponse = await got({
                    method: 'get',
                    url: item.link,
                });

                const c = cheerio.load(detailResponse.data);
                const des = c('.pressContent.full').html();
                const thumbs = c('.col-3-wrap.clearfix.pressPhoto').remove('.btn_download').html() ?? '';
                item.title = c('h2').text().trim();
                item.pubDate = parseDate(decodeURI(c('.date').text().trim()), ['YYYY年MM月DD日', 'YYYY年MM月D日', 'YYYY年M月DD日', 'YYYY年M月D日']);
                item.description = des + thumbs;
                return item;
            })
        )
    );
    ctx.state.data = {
        title: 'ICAC 新闻公布',
        link: 'https://www.icac.org.hk/tc/press/index.html',
        description: 'ICAC 新闻公布',
        item: items,
    };
};
