...
const host = 'https://pgs.ruc.edu.cn';
module.exports = async (ctx) => {
    const { type } = ctx.request.params;
    const pageUrl = `${host}/${type}.htm`;
    const response = await got(pageUrl);
    const $ = cheerio.load(response.data);
    const typeName = $('.col-main .top h3').text() || '研究生招生网';
    const list = $('.col-main li');
    ...
    ctx.state.data = {
        title: `中国人民大学研究生招生网 - ${typeName}`,
        link: pageUrl,
        description: `中国人民大学研究生招生网 - ${typeName}`,
        item: items,
    };
};
