...
module.exports = async (ctx) => {
    ...
    const betterTitle = /t|y/i.test(ctx.params.betterTitle ?? 'false');
        ...
        item:
            ...
            list
                .map((index, item) => {
                    ...
                    let messageText = '',
                        messageTitle = '';
                    const messageHtml = messageTextObject.html();
                    if (messageTextObject.length > 0) {
                        messageText = `<p>${messageHtml}</p>`;
                    }

                    if (betterTitle) {
                        const removeReferenceMessage = messageHtml.replace(/<blockquote>.*?<\/blockquote>/g, '');
                        const boldTitle = removeReferenceMessage.replace(/<(?!b|\/b).+?>/g, '').match(/^(?:\s|<br>)*<b>(.+?)<\/b>/);
                        const clauseTitle = removeReferenceMessage.replace(/<(?!br).+?>/g, '').match(/^(?:\s|<br>)*(.{7,30}?)(?:。|！|!|？|\?|，|,|；|;|<br>|- |_|：(?!“)|:(?!")|(?<![\w\/])\s(?![\w\/]))/);
                        if (boldTitle) {
                            messageTitle = boldTitle[1].replace(/<.+?>/g, '');
                        } else if (pollQuestion !== '') {
                            messageTitle = pollQuestion;
                        } else if (attachmentTitle !== '') {
                            messageTitle = attachmentTitle;
                        } else if (clauseTitle) {
                            messageTitle = clauseTitle[1].replace(/<.+?>/g, '');
                        } else if (messageTextObject.length > 0) {
                            messageTitle = messageTextObject.text().substr(0, 25);
                        } else {
