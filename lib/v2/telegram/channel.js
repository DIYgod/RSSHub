                ...
                .map((index, item) => {
                    ...
                    if (messageTextObj.length > 0 && !titleCompleteFlag) {
                        if (betterTitle) {
                            ...
                            const boldTitle = removeReferenceMessage.replace(/(?:<a.+?>.*?<\/a>|<(?!b|\/b).+?>)/g, '').match(/^(?:\s|<br>)*<b>(.+?)<\/b>/);
                            const clauseTitle = removeReferenceMessage.replace(/(?:<a.+?>.*?<\/a>|<(?!br).+?>)/g, '').match(/^(?:\s|<br>)*(.{5,35})(?:。|！|!|？|\?|，|,|；|;|<br>|- |_|：(?!“)|:(?!")|(?<![\w/])\s(?![\w/]))/);
                            if (boldTitle) {
