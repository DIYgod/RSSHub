...
const apiId = Number(config.apiId ?? 4);
const apiHash = config.apiHash ?? '014b35b6184100b085b0d0572f9b5103';
...
function chooseLargestThumb(thumbs) {
    ...
    thumbs = Array.from(thumbs).sort((a, b) => sortThumb(a) - sortThumb(b));
    return thumbs.pop();
}
...
function streamDocument(obj, thumbSize = '', offset, limit) {
    const requestSize = (obj.size ? getAppropriatedPartSize(obj.size) : 64) * 1024;
