    ...
    items = await Promise.all(
        items.map((item) =>
            ctx.cache.tryGet(item.link, async () => {
                ...
                const pubDate = content('span.pub-time').text() || content('span.fby').text().split('发表于').pop().trim();
                ...
                item.pubDate = timezone(/\d{4}-\d{1,2}-\d{1,2} \d{2}:\d{2}/.test(pubDate) ? parseDate(pubDate, ['YYYY-MM-DD HH:mm', 'YYYY-M-D HH:mm']) : parseRelativeDate(pubDate), +8);
