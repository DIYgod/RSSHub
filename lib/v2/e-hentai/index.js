    ...
    let items = $('table.gltc tbody tr')
        ...
        .map((item) => {
            ...
            return {
                ...
                description: `<img src="${item.find('div.glthumb div img').attr('src')}">`,
                category: item
                    .find('div.gt')
                    .toArray()
                    .map((tag) => $(tag).attr('title').replace(/^:/, '')),
                enclosure_url: item.find('div.gldown a img[title="Show torrents"]').length > 0 ? item.find('.gldown a').attr('href') : undefined,
            };
        });

    items = await Promise.all(
        items.map(async (item) => {
            if (needTorrents && item.enclosure_url) {
                let torrents = await ctx.cache.get(item.enclosure_url);
                ...
                if (!torrents) {
                    ...
                    torrents = torrentResponse.data.match(/<a href="(https:\/\/.*?\.torrent)" onclick=".*; return false">(.*?)<\/a>/g).map((t) => {
                        const match = t.match(/<a href="(https:\/\/.*?\.torrent)" onclick=".*; return false">(.*?)<\/a>/);
                        return { link: match[1], title: match[2] };
                    });
                ...
                item.description += art(path.join(__dirname, 'templates/torrents.art'), { torrents });
            }
