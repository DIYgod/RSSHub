...
module.exports = async (ctx) => {
    ...
    const responses = await Promise.all(
        passagesList.map(async (passage) => {
            // Return and cache responses processed by cheerio.
            const response = await getHTMLWithCache(passage.link, ctx);
            return { ...passage, response };
        })
    );

    for (const passage of responses) {
        const $ = passage.response;
        ...
        [passage.author, passage.pubDate] = getElementChildrenInnerText($('.shijian')[0])
            .split(' ')
            .filter((item) => item !== '')
            .map((item) => item.trim());

        passage.content = $('.contenttext').html();

        passages.push({
            title: passage.title,
            author: passage.author,
            pubDate: timezone(parseDate(passage.pubDate), 8),
            guid: passage.link,
            link: passage.link,
