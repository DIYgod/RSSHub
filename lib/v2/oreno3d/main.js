...
module.exports = async (ctx) => {
    ...
    let limit = ctx.params.maxlimit ?? 1;
    //根链接
    ...
    if ($(maxPageSelector)){
        let pageLink = $(maxPageSelector).attr('href')
        let actualNum = pageLink.split("&page=")[1]
        if (limit > actualNum){limit = actualNum}//超出则用最大的
    }
    else{
        limit = 1 //只有一页
    }
    ...
    ctx.state.data = {
        title: `${title} - ${sortRename[sort]}(Page 1-${limit})`,
        link: userUrl,
        item: items,
    };
};
