...
module.exports = async (ctx) => {
    const resp = await got('https://asa.scitation.org');
    const cookie = resp.headers['set-cookie'].join(' ');
    const hostUrl = 'https://asa.scitation.org/toc/jas/current?size=all';
    const headers = {
        'X-Requested-With': 'XMLHttpRequest',
        'Content-Type': 'application/x-www-form-urlencoded; charset=UTF-8',
        'Accept-Encoding': 'gzip, deflate, br',
        'User-Agent': envs.UA || 'Mozilla/5.0 (Macintosh; Intel Mac OS X 10_13_4) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/65.0.3325.181 Safari/537.36',
        cookie,
    };
