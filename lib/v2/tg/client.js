...
function getMediaLink(ctx, channel, channelName, message) {
    ...
    if (x instanceof Api.MessageMediaDocument && x.document.mimeType.startsWith('video/')) {
        ...
        let poster = '';
        const inlineThumb = x.document.thumbs.find((t) => t.bytes);
        if (inlineThumb) {
            const jpeg = ExpandInlineBytes(inlineThumb.bytes);
            poster = `data:image/jpg;base64,` + jpeg.toString('base64');
        }
        return `<video preload="metadata" poster="${poster}" width="${vid.w}" height="${vid.h}"><source src="${src}" type="${x.document.mimeType}"></video>`;
    }
