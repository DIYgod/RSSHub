...
module.exports = async (ctx) => {
    ...
    const list = initData.data.map((item) => ({
        ...
        reportUrl: `${baseUrl}/report/${linkType[category]}.jshtml?encodeUrl=${item.encodeUrl}`,
    ...
    const items = await Promise.all(
        list.map((item) =>
            ctx.cache.tryGet(item.reportUrl, async () => {
                const { data: response } = await got(item.reportUrl);
