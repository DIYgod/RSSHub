...
const host = 'http://yjsb.just.edu.cn';
module.exports = async (ctx) => {
    ...
    const list = $('.news_list2 .news');
    const items = await Promise.all(
        Array.from(list).map((item) => {
            item = $(item);
            const itemDate = item.find('.news_meta').text();
            const itemTitle = item.find('.news_title').text();
    ...
    ctx.state.data = {
        title: `江苏科技大学研究生院 - ${typeName}`,
        link: pageUrl,
        description: `江苏科技大学研究生院 - ${typeName}`,
        item: items,
    };
};
