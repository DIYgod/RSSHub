...
module.exports = async (ctx) => {
    ...
    let items = $('.nex_forum_lists')
        .toArray()
        .filter((item) => {
            // remove #stickthread
            item = $(item);
            return item.attr('id').includes('normalthread');
        })
        .map((item) => {
            item = $(item);
            const nexAuthorBtms = item.find('.nex_author_btms');
            const nexForumtitTopA = item.find('.nex_forumtit_top a');
    ...
    items = await Promise.all(
        items.map((item) =>
            ctx.cache.tryGet(item.link, async () => {
                ...
                const $ = cheerio.load(resp.data);
                const firstViewBox = $('.viewbox').first();
                firstViewBox.remove('script, .allrepliestit, #reply_order_menu');
