...
module.exports = async (ctx) => {
    ...
    const items = response.data.userSaved.map((item) => ({
        title: item.title,
        description: `原文：${item.metadata.full_text.replace(/\n/gm, '<br/>')}${item.comments.length > 0 ? `<br/><br/>评论：${item.comments[0].text.substring(0, 100)}</br>` : ''}${item.highlights.length > 0 ? `标注：${item.highlights.map((highlight) => (highlight.comment ? `${highlight.highlight}<br/>`+"    "+`评论：${highlight.comment.text}<br/>` : `标注：${highlight.highlight}<br/>`)).join('')}` : ''}`,
        link: item.url,
