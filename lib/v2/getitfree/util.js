...
const bakeFiltersWithPair = async (filters) => {
    ...
    const bakeFilters = async (filters, filtersWithPair) => {
        ...
        return bakeFilters(originalFilters, {
            ...filtersWithPair,
            [key]: filterApiKeysWithNoId.includes(key) ? keywords : await bakeKeywords(key, keywords),
        });
    };
...
const getFilterKeyForSearchParams = (key, isApi = false) => {
    ...
    return keys.hasOwnProperty(key) ? keys[key] ?? key : undefined;
};
...
const parseFilterStr = (filterStr) => {
     ...
     * @returns {Object} The parsed filters object.
     */
    const parseStr = (filterStr, filters = {}, filterKey = undefined) => {
        if (!filterStr) {
            return filters;
        }
        ...
        const isKey = filterApiKeys.hasOwnProperty(word);
