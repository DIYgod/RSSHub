...
const parseAppDate = async (item, ctx) => {
    ...
    if (link_group) {
        try {
            ...
            let app_data = await got(`https://gateway.caixin.com/api/app-api/auth/validate?uid=${uid}&code=${code}&unit=1&articleId=${article_id}&deviceType=5`);
...
const caixinLogin = async (ctx) => {
    ...
    if (uid && code) {
        app_data = await got(`https://gateway.caixin.com/api/app-api/auth/validate?uid=${uid}&code=${code}&unit=1&articleId=102031706&deviceType=5`);
        app_data = JSON.parse(app_data.body);
    }
    if (!app_data || app_data?.data?.info?.errorCode != 0) {
        const login_data = await got(config.caixin.login_url);
