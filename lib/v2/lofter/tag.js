...
module.exports = async (ctx) => {
    ...
    const url = 'https://www.lofter.com/dwr/call/plaincall/TagBean.getCommonTagExcellentAuthors.dwr';
    ...
    const data = context.dwr.engine.data[2];
    ...
    const items = data.map((entry) => {
        ...
        const images = post.photoLinks
            ? JSON.parse(post.photoLinks).reduce((accumulator, currentValue) => accumulator + `<img src="${currentValue.middle}"/>`, '') // small | middle | orign
            : '';

        const digest = cheerio.load(post.digest);
