...
module.exports = async (ctx) => {
    ...
    if (cacheInvalid) {
        ...
        const SEARCH_STRING = 'urlWithParams: "';
        for (const scriptObj of $('script').get()) {
            const scriptText = $(scriptObj).text();
            const index = scriptText.indexOf(SEARCH_STRING);
            if (index > 0) {
                const nextIndex = scriptText.indexOf('"', index + SEARCH_STRING.length);
                dataSourceUrl = AIRTABLE_HOST + scriptText.substring(index + SEARCH_STRING.length, nextIndex).replaceAll('\\u002F', '/');
                break;
            }
        }
