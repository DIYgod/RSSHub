...
module.exports = async (ctx) => {
    ...
    const raw_keyword = (ctx.params.keyword || '').trim();
    const keyword = raw_keyword === '' ? null : decodeURIComponent(raw_keyword);
    ...
    const author_slug = list[0].author.slug;
    const author_nickname = list[0].author.nickname;
    const items = await Promise.all(
        list
            .filter((item) => (keyword ? item.title.includes(keyword) : true))
            .map((item) => {
