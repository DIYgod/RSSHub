...
module.exports = {
    getCookie: async (ctx) => {
        const key = 'cookie'
        let cookie = await ctx.cache.get(key);
        if (!cookie) {
            ...
            ctx.cache.set(key, cookie);
        }
        return cookie;
    },
    getUsernameFromUID: async (ctx, uid) => {
        const cookie = await ctx.cache.get(ctx);
        ...
        if (!name) {
            ...
            const nameResponse = await got({
                method: 'get',
                url: `https://api.bilibili.com/x/space/acc/info?mid=${uid}&token=&platform=web&jsonp=jsonp`,
                headers: {
                    'User-Agent': UA,
