...
module.exports = {
    ...
    getArticleDataFromCvid: async (ctx, cvid, uid) => {
        ...
        if (!description) {
            description = '';
            const wbiVerifyString = await module.exports.getWbiVerifyString(ctx);
            const params = utils.addWbiVerifyInfo(`id=${cvid}&gaia_source=main_web&web_location=333.976`, wbiVerifyString);

            const newFormatUrl = `https://api.bilibili.com/x/article/view?${params}`;
            const cookie = await module.exports.getCookie(ctx);
            const newFormatData = await ctx.cache.tryGet(
                `https://www.bilibili.com/read/cv${cvid}_newFormat`,
                async () =>
                    (
                        await got({
                            method: 'get',
            ...
            if (newFormatData && newFormatData.data && newFormatData.data.opus && newFormatData.data.opus.content && newFormatData.data.opus.content.paragraphs) {
                for (const element of newFormatData.data.opus.content.paragraphs) {
                    if (element.para_type === 1) {
                        for (const text of element.text.nodes) {
                            if (text && text.word && text.word.words) {
                    ...
                    if (element.para_type === 2) {
                        for (const image of element.pic.pics) {
                            description += `<p class="normal-img" contenteditable="false"><img src="${image.url}@progressive.webp" data-status="loaded" alt="read-normal-img" data-w="${image.width}" data-h="${image.height}"></p>`;
                        }
                    }
                    if (element.para_type === 3) {
                        description += `<figure class="img-box" contenteditable="false"><img class="cut-off-5" src="${element.line && element.line.pic && element.line.pic.url}" referrerpolicy="no-referrer"></figure>`;
                    }
                }
            }
        }
