    ...
    getCookie: (ctx) => {
        ...
        return ctx.cache.tryGet(key, async () => {
            ...
            try {
                ...
                cookie.push(['bili_ticket', response.data.data.ticket].join('='), ['bili_ticket_expires', (parseInt(response.data.data.created_at) + parseInt(response.data.data.ttl)).toString()].join('='));
            } catch {
