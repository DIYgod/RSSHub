...
module.exports = async (ctx) => {
    ...
    ctx.state.data = {
        ...
        link: base,
        item: info.map((e, i) => ({
            title: details[i].title || e.title,
            description: utils.renderDesc(details[i].desc),
            pubDate: new Date(details[i].pubDate).toDateString() === 'Invalid Date' ? e.pubDate : details[i].pubDate,
            link: details[i].link_ || e.detailPage,
        })),
    };
};
