    ...
    cache.tryGet(
        ...
        async () => {
            ...
            try {
                ...
                const notesPromise = userPosted.data.notes.map(async (n) => {
                    ...
                    const content = await notePage.content();
                    const feed = JSON.parse(
                        content
                            .match(/<script>window.__INITIAL_STATE__=({.*})<\/script>/g)[0]
                            .match(/{.*}/g)[0]
                            .replace(/undefined/g, '""')
                    );

                    // Sometimes the page is not server-side rendered
                    if (JSON.stringify(feed?.note?.note) === '{}') {
