    ...
    cache.tryGet(
        ...
        async () => {
            ...
            try {
                ...
                const [otherInfo, userPosted] = await Promise.all(
                ...
                const notesPromise = userPosted.data.notes.map((n) => {
                    ...
                    return cache.tryGet(
                        ...
                        async () => {
                            ...
                            const feed = await notePage.evaluate(() => window.__INITIAL_STATE__);
                            ...
                            if (typeof feed?.note?.note !== 'undefined' && JSON.stringify(feed?.note?.note) !== '{}') {
                                ...
                                throw 'Cannot get the content of the note';
                            }
                        },
                        config.cache.routeExpire * 4, // Use a larger expire time for note full text
                        true
                    );
                });
