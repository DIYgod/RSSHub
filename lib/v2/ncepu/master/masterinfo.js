...
module.exports = async (ctx) => {
    ...
    const response = await got({
        method: 'get',
        url: `http://yjsy.ncepu.edu.cn/${url_map[type]}/index.htm`,
    });
    ...
    const list = $('.articleList ul li')
        .map((_, item) => {
            ...
            const a = item.find('a');
            // 发表日期
            const pubDate = timezone(item.find('span').text().replace('[', '').replace(']', ''), +8);
            // 相对地址转化为绝对地址
            const url_head = `http://yjsy.ncepu.edu.cn/${url_map[type]}/`;
    ...
    const items = await Promise.all(
        list.map(async (item) => {
            ...
            const eDescription = content('.pageArticle').html();
