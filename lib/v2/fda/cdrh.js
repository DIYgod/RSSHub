...
module.exports = async (ctx) => {
    ...
    let items = $('div[role="main"] a')
        ...
        .map((item) => {
            ...
            return {
                title: item.text(),
                link: `${/^http/.test(link) ? '' : String(rootUrl)}${link}`,
            };
        });
    ...
    items = await Promise.all(
        items.map((item) =>
            ctx.cache.tryGet(item.link, async () => {
                ...
                item.pubDate = parseDate(content('meta[property="article:modified_time"]').attr('content').split(', ').pop(), 'MM/DD/YYYY - HH:mm');
                item.description = content('div[role="main"]').html();
