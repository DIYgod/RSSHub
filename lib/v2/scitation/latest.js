...
module.exports = async (ctx) => {
    ...
    const host = 'https://'.concat(pub, '.scitation.org');
    const jrnlUrl = host.concat('/toc/', jrn, '/current?size=all');
    ...
    await Promise.all(
        list.map((item) =>
            ctx.cache.tryGet(item.link, async () => {
                const response2 = await got.post(host.concat('/action/PB2showAjaxAbstract'), {
                    cookieJar,
                    body: 'doi=' + item.doi,
                    headers: {
