...
async function getPageSize(ctx, url) {
    const response = await ctx.cache.tryGet(url, async () => {
        const { data } = await got({ method: 'get', url }, { https: { rejectUnauthorized: false } });
        return data;
    });
...
async function getPageUrl(ctx,url, pageId, pageSize) {
    pageId = parseInt(pageId);
    if (pageId && pageId !== 1 && pageId !== 'all') {
...
const getArticleContent = async (ctx, link) => {
    const response = await ctx.cache.tryGet(link, async () => {
        const { data } = await got({ method: 'get', url: link }, { https: { rejectUnauthorized: false } });
        return data;
    });
    const $ = cheerio.load(response);
    ...
    authorInArticle = [...$('.v_news_content').text().matchAll(/[(（]([\W]{3,20}?[\W\s]{2,30}?)[）)]/g)].pop();
...
async function getArticleList(ctx, pageUrl) {
    const response = await ctx.cache.tryGet(pageUrl, async () => {
        const { data } = await got({ method: 'get', url: pageUrl }, { https: { rejectUnauthorized: false } });
        return data;
    });
    const $ = cheerio.load(response);
...
module.exports = async (ctx) => {
    ...
    if (pageId === 'all') {
        for (let i = 1; i <= PageSize; i++) {
            const pageUrl = await getPageUrl(ctx, url, i, PageSize);
            const pageItems = await Promise.all(await getArticleList(ctx, pageUrl));
            items = items.concat(pageItems);
        }
    } else {
        const pageUrl = await getPageUrl(ctx, url, pageId, PageSize);
        const pageItems = await Promise.all(await getArticleList(ctx, pageUrl));
        items = items.concat(pageItems);
    }
