        ...
        list.map(async (item) => {
            ...
            const threadPromises = [item.threadData1URL, item.threadData2URL].map((url) =>
                ctx.cache.tryGet(item.link, async () => {
                    ...
                    const threads = data.tweets ? data.tweets : data;
                    item.threads = [...item.threads, ...threads];
                    return item;
                })
            );
            await Promise.all(threadPromises);
            item.description = item.threads.reduce((accumulator, tweet) => `${accumulator}${tweet.tweet_detail.info.text}<br>`, '');
            return item;
        })
    );
