    ...
    const all = await Promise.all(
        types.map(async (type) => {
            const response = await got({
                method: 'get',
                url: `https://docs.rsshub.app/${lang}routes/${type}`,
            });
    ...
    ctx.state.data = {
        ...
        item: list.map(({ page, item, type }) => {
            ...
            return {
                ...
                link: `https://docs.rsshub.app/${lang}routes/${type}#${encodeURIComponent(titleEle.find('.hash-link').attr('href') && titleEle.find('.hash-link').attr('href').slice(1))}`,
                guid: item.attr('id'),
            };
        }),
    };
