...
async function getItems(ctx, url, host, tableClass, timeStyleClass, titleStyleClass) {
    // 发起Http请求，获取网页数据
    ...
    const list = $(`table.${tableClass} > tbody > tr`)
        .filter((index, item) => {
            const currentItem = $(item);
            const item1 = currentItem.find('td:eq(1)');
            const hasLink = item1.find('a').length > 0;
            const isLastItem = index === $(`table.${tableClass} > tbody > tr`).length - 1;
        ...
        .map((item) => {
            ...
            return {
                ...
                link: link.replace('..', ''),
                pubDate: timezone(parseDate(item2.find(`.${timeStyleClass}`).text(), 'YYYY-MM-DD'), +8),
            };
        });
            ...
            ctx.cache.tryGet(item.link, async () => {
                ...
                if (response.redirectUrls.length) {
                    ...
                    item.description = $('.v_news_content')
                        ...
                        .trim();
                    item.pubDate = timezone(parseDate($(`.${timeStyleClass}`).text().replace('发布时间：', '')), +8);
                }
                return item;
            })
        )
