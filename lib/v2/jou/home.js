...
module.exports = async (ctx) => {
    ...
    const list = $('table.winstyle106390 > tbody > tr')
        .filter((index, item) => {
            const currentItem = $(item);
            const item1 = currentItem.find('td:eq(1)');
            const hasLink = item1.find('a').length > 0;
            const isLastItem = index === $('table.winstyle106390 > tbody > tr').length - 1;
        ...
        .map((item) => {
            ...
            return {
                ...
                link: link.replace('..', ''),
                pubDate: timezone(parseDate(item2.find('.timestyle106390').text(), 'YYYY-MM-DD'), +8),
            };
        });
