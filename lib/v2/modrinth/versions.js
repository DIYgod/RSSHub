...
module.exports = async (
    ...
    try {
        ...
        const versions = await got(
            `https://api.modrinth.com/v2/project/${id}/version?${loaders ? `loaders=${JSON.stringify(loaders)}` : ''}${game_versions ? `&game_versions=${JSON.stringify(game_versions)}` : ''}&${featured === undefined ? '' : `&featured=${featured}`}`
        ).json();
        /**
         * @type {import('./api').Author[]}
         */
        const authors = await got(`https://api.modrinth.com/v2/users?ids=${JSON.stringify(versions.map((it) => it.author_id))}`).json();
