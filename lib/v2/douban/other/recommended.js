...
module.exports = async (ctx) => {
    const subject_type = ctx.params.type || 'tv';
    const apikey = '0ac44ae016490db2204ce0a042db2916';
    let url = `https://frodo.douban.com/api/v2/skynet/new_playlists?apikey=${apikey}&subject_type=${subject_type}`;
    ...
    let subject_collection_id;
    const date = new Date();
    const year = date.getFullYear();
    const mon = date.getMonth() + 1;
    let items = response.data.data[0].items;

    subject_collection_id = items.find((item) => item.title.startsWith(`${year}年${mon}月`)).id;
    ...
    url = `https://m.douban.com/rexxar/api/v2/subject_collection/${subject_collection_id}/items?playable=${playable}`;
    response = await got({
        method: 'get',
        url,
        headers: {
            Referer: `https://m.douban.com/subject_collection/${subject_collection_id}`,
        },
    });
    ...
    ctx.state.data = {
        ...
        link: `https://m.douban.com/subject_collection/${subject_collection_id}`,
        item: items,
        description,
    };
};
