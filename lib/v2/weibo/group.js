...
module.exports = async (ctx) => {
    ...
    let resultItems = await Promise.all(
        responseData.statuses.map(async (item) => {
            // const key = 'weibo:user:' + item.id
            // const data = await ctx.cache.tryGet(key, () => )

            const retweet = item.retweeted_status;
            if (retweet && retweet.isLongText){
                const rewteetData = await ctx.cache.tryGet(`weibo:retweeted:${retweet.user.id}:${retweet.bid}`, () => weiboUtils.getShowData(retweet.user.id, retweet.bid));
            ...
            if (displayComments === '1') {
                description = await weiboUtils.formatComments(ctx, descripion, item);
            }
