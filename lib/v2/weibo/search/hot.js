...
async function fetchContent(url) {
    // Mock the url!
    // From 'search' to 'getindex'
    // "m.weibo.cn/search?" => "m.weibo.cn/api/container/getIndex"
    ...
    try {
        ...
        for (let i = 0; i < cards.length; i++) {
            const item = cards[i];
            if (item.card_type === 9) {
                demostr = demostr + '<div>' + item.mblog.text + '</div>';
                demostr = demostr + '<div>-picnum=' + item.mblog.pic_num + '-</div>';
            }
        }
    } catch (e) {
        // console.log(e);
