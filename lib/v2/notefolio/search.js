...
module.exports = async (ctx) => {
    // 分类、排序、时间范围、搜索词
    const { category = 'all', order = 'like', time = 'all', query = '' } = ctx.params;
    //  分页
    const { limit } = ctx.query;

    // 请求链接
    let searchUrl = `https://api.stunning.kr/api/v1/dantats/portfolio?state=Public&limit=${limit ? parseInt(limit, 10) : 20}&search=${query}&orderBy=${order}`;
    // 分类
    const index = (Number(category) || 1) - 1;
    if (!category && index >= 0 && categoryMap[index]) {
        ...
        data?.resultData.map((item) => {
            ...
            const description = contents
                .map((m) => {
                    ...
                    if (type === 'Image') {
                        text = m.files.map((f) => `<img src='${f.url}' alt style="max-width: 100%; height: auto;">`);
                    } else if (type === 'Text') {
                        text = m.content;
                    } else if (type === 'Video') {
                        text = `<iframe src='${m.url}'></iframe>`;
                    }
