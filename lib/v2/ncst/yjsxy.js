...
module.exports = async (ctx) => {
    ...
    const typeName = $('.dqwz_box a').last().text() || '研究生院';
    const list = $('.lis-hover');
    const items = await Promise.all(
        Array.from(list).map((item) => {
            item = $(item);
            const aTag = item.find('a');
            const titleDate = aTag.text();
            const itemDate = titleDate.slice(-10);
            const itemTitle = titleDate.slice(0, -10);
