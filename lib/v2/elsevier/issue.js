...
module.exports = async (ctx) => {
    ...
    const issueUrl = host.concat('/journal/', journal, '/vol/', ctx.params.issue.replace('-', '/issue/'));
    ...
    await Promise.all(
        list.map((item) =>
            ctx.cache.tryGet(item.link, async () => {
                const response2 = await got(host.concat('/science/article/pii/', item.id), {
                    cookieJar,
                });
