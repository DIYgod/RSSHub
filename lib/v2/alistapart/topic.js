    ...
    const listItems = $('main.site-main article')
        ...
        .map((item) => {
            ...
            const author = item
                ...
                .map((item) => $(item).text())
                .join(', ');
            const pubDate = parseDate(item.find('span.posted-on time.entry-date').attr('datetime'));
            const updated = parseDate(item.find('span.posted-on time.updated').attr('datetime'));
            return {
                title: a.text(),
