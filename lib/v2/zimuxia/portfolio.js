...
const allowlinktypes = [ 'all', 'magnet', 'ed2k', 'baidu', 'subhd', 'quark', '115' ];

module.exports = async (ctx) => {
    const id = ctx.params.id;
    let linktype = ctx.query.linktype ?? 'magnet';
    linktype = allowlinktypes.includes(linktype) ? linktype : 'magnet';
    ...
    const items = $('a')
        ...
        .map((item) => {
            ...
            const tmphtml = item.parent().contents().toArray().map((item) => $.html($(item)));
