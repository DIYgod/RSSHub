...
function ParseDownloadLinks(html, defaultDrive) {
    ...
    return links.slice(0, Math.floor(links.length / 2)).map((link) => {
        ...
        if (href.includes('rosefile')) {
            ...
            text = '77File';
        } else if (href.includes('stfly') || href.includes('shrinkme') || href.includes('ouo') || href.includes('link1s')) {
            text = defaultDrive;
        } else {
            text = '未知下载方式';
        }
...
async function ProcessItems(ctx, content) {
...
module.exports = {
    ProcessUrl: async (url, ctx) => {
        try {
            ...
            const newItems = await ProcessItems(ctx, content);
            return newItems;
        } catch {
