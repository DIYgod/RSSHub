...
module.exports = async (ctx) => {
    ...
    const $mobileHeader = $('#msite-pages-header-contents');

    if ($mobileHeader.length > 0) {
        const title = $mobileHeader.find('div[data-nt="FB:TEXT4"]').eq(0).text();
        ...
        const page = await ctx.cache.tryGet(
            ...
            async () => {
                ...
                const title = $desktop('h1').text();
                const description = $desktop('div[data-pagelet="ProfileTilesFeed_0"] > div  > div  > div  > div > div:eq(1) > div:eq(0)').text();
                const itemLinks = $desktop('span > a')
                    .toArray()
                    .map((a) => $(a).attr('href'))
                ...
                return {
                    title,
                    description,
                    items: items.filter((item) => item.title !== '' && item.description !== ''),
                };
            },
        ...
        ctx.state.data = {
            ...
            item: page.items.filter((item) => !!item),
        };
    }
};
