...
module.exports = async (ctx) => {
    ...
    const category_item = category_map[category] || null;
    let page_url;
    if (category_item) {
        page_url = [`${base_url}${category_item.link}/list.htm`];
    } else {
        page_url = Object.values(category_map).map((item) => `${base_url}${item.link}/list.htm`);
    }
