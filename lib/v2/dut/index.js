...
module.exports = async (ctx) => {
    ...
    items = items
        ...
        .map((item) => {
            ...
            if (site === 'fldpj') {
                itemTitle = item.find('em').text();
                itemDate = item.find('span').text();
            } else {
                ...
                itemTitle = item.text() === '' ? item.next().text() : item.text();
                itemDate = dateMatch ? dateMatch[1].replace(/年|月/g, '-') : new Date().toString();
            }

            const itemLink = /^http/.test(item.attr('href')) ? item.attr('href') : `${rootUrl}/${item.attr('href').replace(/^[./]+/, '')}`;
