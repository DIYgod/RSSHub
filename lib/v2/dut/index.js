...
module.exports = async (ctx) => {
    ...
    let items = $('.txt, .itemlist, .wall, .list, .list01, .ny_list, .rjxw_right, .rj_yjs_con, .c_hzjl_list1')
        .find('a')
        ...
        .map((item) => {
            ...
            return {
                ...
                link: item.attr('href').indexOf('http') === 0 ? item.attr('href') : `${rootUrl}/${item.attr('href').replace(/\.\.\//g, '')}`,
