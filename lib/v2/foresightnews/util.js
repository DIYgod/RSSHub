...
const processItems = async (apiUrl, limit, ...searchParams) => {
    ...
    items = (items?.list ?? items).slice(0, limit).map((item) => {
        const sourceType = item.source_type ?? (item.source_link ? 'news' : item.event_type ? 'event' : constants.defaultType);
        ...
        const categories = [column, item.event_type, item.is_hot ? constants.labelHot : undefined, item.is_important ? item.important_tag?.name ?? constants.labelImportant : '', item.label, ...tags].filter(
