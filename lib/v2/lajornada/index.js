...
const getDateForToday = () => {
    ...
    const today = date.getFullYear() + '-' + ('0' + date.getMonth()).slice(-2) + '-' + ('0' + date.getDay()).slice(-2);
    return today;
};

module.exports = async (ctx) => {
    const date = ctx.params.date === 'today' ? getDateForToday() : ctx.params.date;
    ...
    if (!category) {
        ...
        const newsFilteredByCategory = data.filter((item) => {
