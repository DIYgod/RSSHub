...
module.exports = async (ctx) => {
    const listPage = await got.get('http://wlwz.changsha.gov.cn/webapp/cs2020/email/index.jsp', {
        responseType: 'buffer',
    });
        ...
        list.map((item) =>
            ctx.cache.tryGet(item.link, async () => {
                ...
                const data = {
                    ...
                    pubDate: new Date($('.letter-details div:first table tr:nth-child(2) > .td_label2').text() + ' GMT+8'),
                    link: item.link,
                    author: item.author,
                };
                return data;
