...
//         title_element: '.content_title',                             // 正文的标题。
...
const gdgov = async (info, ctx) => {
    ...
    const list_include = info.list_include;
    ...
    // 判断是否处于特殊目录
    if (path[0] === 'gkmlpt') {
        list_element = '.first-td a';
        title_element = '.title';
        title_match = '(.*)';
        description_element = 'div[class="article-content"]';
        pubDate_element = '.date-row';
        pubDate_match = '发布日期：(.*)  浏览次数';
    ...
    const items = await Promise.all(
        list.map(async (i, item) => {
            ...
            // http://mmswj.maoming.gov.cn/zcjdpt?id=4583
            // https://zcjd.cloud.gd.gov.cn/api/home/article?id=4583
            if (idlink.pathname === '/zcjdpt') {
                ...
                await ctx.cache.tryGet(link, async () => {
                    ...
                    // 关闭浏览器进程
                    browser.close();
                    content = cheerio.load(html); // 使用 cheerio 加载返回的 HTML
                });
                author = content('.department').text();
                pubDate = content('.pub_time').text().trim().match('发布时间：(.*)')[1];

                title = content('.policy').text();
            ...
            if (!isswitch) {
                ...
                description = content(description_element).html();
            }
            return {
                title,
                description,
