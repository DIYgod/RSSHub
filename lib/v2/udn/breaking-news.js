    ...
    const items = await Promise.all(
        response.data.lists.map(async (item) => {
            ...
            const { author, description } = await ctx.cache.tryGet(link, async () => {
                ...
                const vip = result.data.match(/<script language=javascript>window\.location\.href="(https:\/\/vip\.udn\.com[^"]+")/);
                if (vip !== null) {
                ...
                return {
                    ...
                    description: (figure.length > 0 ? figure.html() : '') + $('.article-content__editor').html(),
                };
            });
