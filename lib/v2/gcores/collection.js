...
module.exports = async (ctx) => {
    // get params
    const { collection, category } = ctx.params;
    // make a request
    const url = `https://www.gcores.com/collections/${collection + (category ? `?tab=${category}` : '')}`;
    const res = await got({
        method: 'get',
        url,
    });
    ...
    const out = await Promise.all(
        list.map((item) => {
            ...
            return ctx.cache.tryGet(articleUrl, async () => {
                ...
                let articleData = await got(`https://www.gcores.com/gapi/v1${item.url}`);
