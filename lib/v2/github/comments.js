...
module.exports = async (ctx) => {
    ...
    timeline.forEach((item) => {
        switch (item.event) {
            ...
            case 'committed':
                items.push({
                    title: `${item.committer.name} ${item.event} on ${user}/${repo}: ${typeDict[type].title} #${issue.number}`,
            ...
            case 'head_ref_force_pushed':
                items.push({
                    title: `${item.actor.login} ${item.event.replace(/_/g, ' ')} on ${user}/${repo}: ${typeDict[type].title} #${issue.number}`,
                    author: item.actor.login,
                    pubDate: parseDate(item.created_at),
                    link: item.url,
                });
            ...
            case 'labeled':
                items.push({
                    title: `${item.actor.login} ${item.event} on ${user}/${repo}: ${typeDict[type].title} #${issue.number}`,
                    description: `${item.actor.login} added the ${item.label.name} label`,
                    author: item.actor.login,
                    pubDate: parseDate(item.created_at),
                    link: item.url,
                });
            ...
            case 'review_requested':
                items.push({
                    title: `${item.actor.login} ${item.event.replace('_', ' ')} on ${user}/${repo}: ${typeDict[type].title} #${issue.number}`,
                    description: `${item.review_requester.login} requested a review from ${item.requested_reviewer.login}`,
                    author: item.actor.login,
                    pubDate: parseDate(item.created_at),
                    link: item.url,
                });
