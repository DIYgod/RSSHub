...
async function singleIssue(ctx, user, repo, number, limit, headers) {
    ...
    timeline.forEach((item) => {
        ...
        switch (item.event) {
            ...
            case 'committed':
                items.push({
                    title: `${item.author.name} authored and ${item.committer.name} committed on ${user}/${repo}: ${typeDict[type].title} #${issue.number}`,
                    description: item.message,
                    pubDate: parseDate(item.committer.date),
                    link: item.html_url,
                });
                break;
            case 'convert_to_draft':
                items.push({
                    title: `${actor} converted to draft on ${user}/${repo}: ${typeDict[type].title} #${issue.number}`,
                    description: `${actor} marked this pull request as draft`,
                    author: actor,
                    pubDate: parseDate(item.created_at),
                    link: item.url,
                });
            ...
            case 'head_ref_deleted':
                items.push({
                    title: `${actor} deleted on ${user}/${repo}: ${typeDict[type].title} #${issue.number}`,
                    description: `${actor} deleted the <code>HEAD</code> branch`,
                    author: actor,
                    pubDate: parseDate(item.created_at),
                    link: item.url,
                });
            ...
            case 'head_ref_force_pushed':
                items.push({
                    title: `${actor} force-pushed on ${user}/${repo}: ${typeDict[type].title} #${issue.number}`,
                    description: `${actor} force-pushed the <code>HEAD</code> branch`,
                    author: actor,
                    pubDate: parseDate(item.created_at),
                    link: item.url,
                });
            ...
            case 'head_ref_restored':
                items.push({
                    title: `${actor} restored on ${user}/${repo}: ${typeDict[type].title} #${issue.number}`,
                    description: `${actor} restored the <code>HEAD</code> branch`,
                    author: actor,
                    pubDate: parseDate(item.created_at),
                    link: item.url,
                });
            ...
            case 'labeled':
                items.push({
                    title: `${actor} ${item.event} on ${user}/${repo}: ${typeDict[type].title} #${issue.number}`,
                    description: `${actor} add the <code>${item.label.name}</code> label`,
                    author: actor,
                    pubDate: parseDate(item.created_at),
                    link: item.url,
                });
            ...
            case 'merged':
                items.push({
                    title: `${actor} ${item.event} on ${user}/${repo}: ${typeDict[type].title} #${issue.number}`,
                    description: `${actor} merged commit <b>${item.commit_id}</b>`,
                    author: actor,
                    pubDate: parseDate(item.created_at),
                    link: item.url,
                });
            ...
            case 'ready_for_review':
                items.push({
                    title: `${actor} marked as ready for review on ${user}/${repo}: ${typeDict[type].title} #${issue.number}`,
                    description: `${actor} marked this pull request as ready for review`,
                    author: actor,
                    pubDate: parseDate(item.created_at),
                    link: item.url,
                });
            ...
            case 'unlabeled':
                items.push({
                    title: `${actor} ${item.event} on ${user}/${repo}: ${typeDict[type].title} #${issue.number}`,
                    description: `${actor} removed the <code>${item.label.name}</code> label`,
                    author: actor,
                    pubDate: parseDate(item.created_at),
                    link: item.url,
                });
