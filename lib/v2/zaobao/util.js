...
const orderContent = (parent) => {
    for (const [i, e] of parent
        ...
        .sort((a, b) => {
            ...
            a = Buffer.from(base32.parse(parent.find(a).data('s').slice(Math.max(0, index)))).toString();
            b = Buffer.from(base32.parse(parent.find(b).data('s').slice(Math.max(0, index)))).toString();
            return a - b;
        })
        .entries()) {
