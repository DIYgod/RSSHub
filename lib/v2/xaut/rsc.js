...
module.exports = async(ctx) => {
    ...
    const list = $('.vsb-space.n_right .list .cleafix')
        .map((_, item) => {
            //  $('.vsb-space.n_right .list .cleafix').eq(0).find('.list_wen a').eq(0).attr('href')
            item = $(item);
            ...
            // 不知道这里对日期格式有没有要求，网页上的日期不包含年份
            // 答：做了要求
            const date = '2021-' + item.find('.list_time p').eq(1).text().slice(0, -1) + '-' + item.find('.list_time p').eq(0).text();
            ...
            list.map((item) =>
                ctx.cache.tryGet(item.link, async() => {
                    ...
                    if (item.link.slice(0, 16) === 'http://renshichu') {
                        ...
                        let time_ = content('.vsb-space form[name] h3 span:contains(时间)').text().slice(3, );
                        item.pubDate = time_
                            // 年份：
