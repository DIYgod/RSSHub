...
module.exports = async (ctx) => {
    ...
    const out = await Promise.all(
        items.map(async (index, item) => {
            ...
            const { description, author } = await ctx.cache.tryGet(link, async () => await extractor(link, ctx));
            const span = item.find('span');
            const pubDate = parseDate(span.text(), 'YYYY/MM/DD');
            return {
                title,
                link,
                description,
    ...
    ctx.state.data = {
        ...
        link: 'https://xinwen.wh.sdu.edu.cn/',
        item: out,
    };
};
