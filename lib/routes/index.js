...
try {
    ...
    gitHash = (process.env.HEROKU_SLUG_COMMIT?.slice(0, 7)) || (process.env.VERCEL_GITHUB_COMMIT_SHA?.slice(0, 7)) || 'unknown';
}
...
export default async (ctx) => {
    ...
    ctx.body = art(path.resolve(dirname, '../views/welcome.art'), {
        showDebug,
        disallowRobot,
        debug: [
            config.nodeName
            ...
            {
                name: 'Memory Usage',
                value: `${stats.memory / 1_000_000} MB`,
            },
            {
                name: 'CPU Usage',
                value: stats.cpu + '%',
            ...
            {
                name: 'Run Time',
                value: (stats.elapsed / 3_600_000).toFixed(2) + ' hour(s)',
            },
            {
                name: 'Hot Routes',
