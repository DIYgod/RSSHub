...
module.exports = async (ctx) => {
    ...
    const getCountry = () => {
        ...
        item.title = `国内累计确诊 ${META.confirmedCount} 例（较昨日 ${(META.confirmedIncr > 0 ? '+' : '') + META.confirmedIncr}），现存确诊 ${META.currentConfirmedCount} 例（较昨日 ${(META.currentConfirmedIncr > 0 ? '+' : '') +
            META.currentConfirmedIncr}），累计死亡 ${META.deadCount} 例（较昨日 ${(META.deadIncr > 0 ? '+' : '') + META.deadIncr}），累计治愈 ${META.curedCount} 例（较昨日 ${(META.curedIncr > 0 ? '+' : '') + META.curedIncr}）`;
    };
