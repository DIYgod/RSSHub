...
module.exports = async (ctx) => {
    ...
    let oldData, file=fs.readFileSync(dataPath).toString();
    if (file != "") {
    ...
    if (Statistics != undefined && ListByCountryTypeService1 != undefined) {//检查是否获取成功
        let pubDate = (new Date((function (s) {
        ...
        function countAnalyze(raw) {
            ...
            let a = /确诊 ([0-9]+) 例 疑似 ([0-9]+) 例 治愈 ([0-9]+) 例 死亡 ([0-9]+) 例/g.exec(raw)
            return a ? {
                确诊: a[1],
                疑似: a[2],
                治愈: a[3],
        ...
        items.push({
            ...
            距离上次更新:${(function(oldTime,newTime){
                    let ta=((newTime-oldTime)/60000).toString().slice('.')
                ...
                <br>${(function (oldData,newDataC,countAnalyze){
                    ...
                    let oldDataC=oldData._count, str="较上次更新:"
                    for (i in newDataC){
                        let t=newDataC[i]-oldDataC[i];
                        if(t!=0){str+=`${i}:${t}`;}
                    }
                    return str
        ...
        let subscribedProvinces=ctx.params['province'];
                    if(subscribedProvinces){
                        ...
                        function convertFromTags(tags){
                                let content=tags.split(/,|，/),dataProccessed={unknown:""}
                                 if(!(content.length>1)){//兼容空格分隔
                                    content=tags.split(' ')
                                    //sample:确诊 3 例 疑似 115 例
                                    let strArray=[],unknown=content
                                    const TO_DELETE="_TO_DELETE_"
                                    for(charNo in content){
                                ...
                                for(i in content){
                                    let regExResult=/([\S]+) ([0-9]+)/.exec(content[i])//OO {num} 型
                                    if(regExResult){
                        ...
                        for (item in subscribedProvinces){
                           
                            for(id in dataProvince){
                            ...
                            if(now[subscribedProvinces[item]]){
                              let nowDataProccessed=now[subscribedProvinces[item]],oldDataProccessed=old[subscribedProvinces[item]],differ={},strArray=[]
                               for(dataName in nowDataProccessed){
                                 if(dataName!="unknown"){
