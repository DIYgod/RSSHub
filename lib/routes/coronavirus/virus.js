...
module.exports = async (ctx) => {
    ...
    const items = [],
        StatisticsRaw = getJSON($('script#getStatisticsService').html()),
        ListByCountryTypeService1Raw = getJSON($('script#getListByCountryTypeService1').html());

    if (StatisticsRaw !== undefined && ListByCountryTypeService1Raw !== undefined) {
        // 检查是否获取成功
        const Statistics = JSON.parse(StatisticsRaw),
            ListByCountryTypeService1 = JSON.parse(ListByCountryTypeService1Raw);
        const pubDate = new Date(
            (function(s) {
                if (s.modifyime === undefined) {
                    // 处理打字错误
