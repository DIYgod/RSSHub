...
module.exports = async (ctx) => {
    ...
    const items = [],
        ...
        ListByCountryTypeService1Raw = getInner($('script#getListByCountryTypeService1').html());
    // const AreaStatRaw=getInner($('script#getAreaStat').html());

    if (StatisticsRaw && ListByCountryTypeService1Raw) {
        // 检查是否获取成功
        const Statistics = JSON.parse(StatisticsRaw),
            ListByCountryTypeService1 = JSON.parse(ListByCountryTypeService1Raw);
            ...
            description: `<pre>距离上次更新:${(function(oldTime, newTime) {
                ...
                ${(function(dataProvince, oldDataProvince) {
                    ...
                    if (subscribedProvinces) {
                        ...
                        const now = {},
                            old = {};
                        const convertFromTags = function(tags) {
                            ...
                            if (!(content.length > 1)) {
                                ...
                                if (deletedUnknown.length > 0) {
                                    strArray.push(deletedUnknown.join(' '));
                                }
                                content = strArray;
                            }
                        ...
                        for (const item in subscribedProvinces) {
                            if (!checkHanzi(subscribedProvinces[item])) {
                                break;
                            }
