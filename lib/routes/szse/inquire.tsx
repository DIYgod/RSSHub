...
async function handler(ctx) {
    ...
    const items = data.data.map((item) => {
        ...
        return {
            ...
            description: renderInquireDescription(item),
        };
    });
...
const renderInquireDescription = (item): string =>
    renderToString(
        <>
            <p>公司代码：{item.gsdm}</p>
            <p>公司简称：{item.gsjc}</p>
            ...
            <p>函件类别：{item.hjlb}</p>
            <p>
                函件内容：<a href={`http://reportdocs.static.szse.cn/${item.ck}`}>详细内容</a>
            </p>
            <p>公司回复：{raw(item.hfck)}</p>
        </>
    );
