...
function getUserNickname(userId) {
    try {
        const userInfoResponse = get(`/user/${userId}`);
...
function getTokenName(tokenId) {
    try {
        const tokenInfoResponse = get(`/minetoken/${tokenId}`);
 ...
 * @param {number} postId
 */
function getPostIpfsHtmlHash(postId) {
    const ipfsInfoResponse = get(`/p/${postId}/ipfs`);
 ...
 * @param {Object} item
 */
function postToIpfsFeedItem(item) {
    const ipfsHtmlHash = getPostIpfsHtmlHash(item.id);
 ...
 */
function getPostsAsFeedItems(url, ipfsFlag) {
