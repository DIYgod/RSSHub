...
module.exports = async (ctx) => {
    ...
    const res = await util.instance.get(util.domain + '/hot.html');
    const data = iconv.decode(res.data, util.encoding);
    const $ = cheerio.load(data);
    let albumLink = $('.td3 div a').map((i, el) => util.domain + el.attribs.href).toArray();
    let albumName = $('.td3 div a').map((i, el) => el.attribs.title).toArray();
    if (count !== 100) {
    ...
    ctx.state.data = {
        title: '性感美女屋 Hot' + count,
        link: util.domain + '/hot.html',
        description: '性感美女屋 Hot' + count,
        item: resultItem
    };
};
