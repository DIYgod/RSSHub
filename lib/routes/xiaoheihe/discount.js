...
module.exports = async (ctx) => {
    ...
    ctx.state.data = {
        ...
        item: data.map((item) => ({
            ...
            description: `<img src="https://steamcdn-a.akamaihd.net/steam/apps/${item.appid}/header.jpg" /> <br> 原价￥${item.heybox_price.original_coin / 1000}元，现价￥${item.heybox_price.cost_coin / 1000}元，折扣力度${item.heybox_price.discount}%OFF，${item.price.deadline_date} <br> 评分：${item.score} <br> 简介：${item.about_the_game}`,
