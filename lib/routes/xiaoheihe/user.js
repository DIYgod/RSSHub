...
module.exports = async (ctx) => {
    ...
    const response = await got({
        method: 'get',
        url: `https://api.xiaoheihe.cn/bbs/web/profile/post/links?userid=${id}&limit=${limit}&offset=0&version=999.0.0`,
    });
    ...
    const items = await Promise.all(
        articleList.map((item) =>
            ctx.cache.tryGet(item.link, async () => {
                ...
                if (video) {
                    description = `<img src = "${video.thumb}"><p><a href ="${video.url_info.url}">video</a></p>`;
                }
