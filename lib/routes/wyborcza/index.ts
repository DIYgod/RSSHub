...
const rootUrl = 'https://wyborcza.pl';
const feedUrl = 'https://wyborcza.pl/pub/rss/najnowsze_wyborcza.xml';

export const route: Route = {
    path: '/',
    categories: ['traditional-media'],
    example: '/wyborcza',
    radar: [{ source: ['wyborcza.pl/', 'wyborcza.pl/*'], target: '/' }],
    name: 'News',
    maintainers: ['lisyer'],
    url: 'wyborcza.pl/',
    handler,
...
async function handler(ctx) {
    ...
    return {
        title: feed.title || 'Gazeta Wyborcza',
        link: feed.link || rootUrl,
        description: feed.description,
        language: feed.language || 'pl',
        item: items,
    };
}
