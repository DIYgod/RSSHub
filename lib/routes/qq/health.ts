...
const url = 'https://news.qq.com/ch/health/';
const api = 'https://i.news.qq.com/web_feed/getCollectionArticles?tagId=aEGsxLNdhWU%3D';

const headers = {
    'user-agent':
        'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/135.0.0.0 Safari/537.36',
    accept: 'application/json, text/plain, */*',
    referer: 'https://news.qq.com/ch/health/',
...
export const route: Route = {
    ...
    handler: async () => {
        ...
        const articleList = response.data?.data?.articleList ?? [];

        if (!articleList.length) {
            throw new Error('QQ health getCollectionArticles returned empty articleList');
        }
