    ...
    const list = $('.thread_every_container_div')
        .map((_, item) => {
            ...
            return {
                ...
                pubDate: parseDate(dateStr, 'YY-M-D H:mm'),
            };
        })
        .get();
    const items = await Promise.all(
        list.map((item) =>
            ctx.cache.tryGet(item.link, async () => {
                ...
                item.description = content('.zhengwen_div:first').text().replace('Your browser does not support HTML5, please change another browser.', '');
                return item;
            })
        )
    );
