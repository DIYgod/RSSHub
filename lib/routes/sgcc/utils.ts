...
type JsonObject = { [key: string]: string | number | boolean | null | JsonObject };
...
const sm4Encrypt = (payload: JsonObject, uuid: string) => {
...
const request = async (path: string, payload: JsonObject, uuid: string, publicKey: string, headers?: Record<string, string>) => {
...
export async function createClient() {
    ...
    return {
        post: (path: string, payload: JsonObject, headers?: Record<string, string>) => request(path, payload, uuid, publicKey, { keyCode, ...headers }),
    };
}
