...
module.exports = async (ctx) => {
    ...
    for (const date of dateList) {
        const cache_key = `iciba-daily-sentence-${date}`;
        cacheData.push(ctx.cache.get(cache_key));
    }
    ...
    for (let x = 0; x < days; x++) {
        if (!cacheData[x]) {
            newData.push(
                got({
                    method: 'get',
                    url: `http://open.iciba.com/dsapi/?date=${dateList[x]}`,
                })
            );
            newId.push(x);
        }
    ...
    for (const id of newId) {
        const dailyData = newData[newId.indexOf(id)].data;
        const cache_key = `iciba-daily-sentence-${dateList[id]}`;
        data[id] = dailyData;
        ctx.cache.set(cache_key, dailyData);
    }
    ...
    ctx.state.data = {
        ...
        image: `http://cdn.iciba.com/www/img/www/0312/ciba-icon.png`,
        item: data.map((item) => ({
            // 文章标题
            title: item.content,
            // 文章正文
