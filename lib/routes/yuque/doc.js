...
module.exports = async (ctx) => {
    ...
    ctx.state.data = await Promise.all([fetchData(repoUrl), fetchData(docsUrl)]).then(async ([{ data: { data: { name: repo, user: { name }, description } } }, { data: { data: docs } }]) => {
        // 过滤掉草稿类型
