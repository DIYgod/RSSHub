        ...
        const data = await Promise.all(
            response.data.statuses.map(async (item) => {
                const realContent = await ctx.cache.tryGet('weibotimelineurl' + item.user.id + item.id, async () => {
                    ...
                    try {
                        if (res.length >= 2) {
                            content = `${res[0]}<br><br><hr style="height:5px;border:none;border-top:5px ridge green;" /><span style="background-color:#71adec">${
                                item.retweeted_status.user.screen_name
                            }:<hr style="height:1px;border:none;border-top:1px solid #555555;" /><br>${res[1]}</span>`;
