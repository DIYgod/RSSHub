...
module.exports = async (ctx) => {
    ...
    const getTime = function (timestr) {
        ...
        if (timestr.includes('分钟前')) {
            now.setMinutes(now.getMinutes() - Number(timestr.split('分钟')[0]));
            timestr = `${now.getMonth() + 1}月${now.getDate()}日 ${now.getHours()}:${now.getMinutes()}`;
        }
        if (timestr.includes('秒前')) {
            now.setSeconds(now.getSeconds() - Number(timestr.split('秒')[0]));
            timestr = `${now.getMonth() + 1}月${now.getDate()}日 ${now.getHours()}:${now.getMinutes()}`;
        }
    ...
    for (let i = 0; i < columns.length; i++) {
        if ($(columns[i]).text().includes('精选内容') || $(columns[i]).text().includes('全部微博')) {
            ...
            list
                .map((index, item) => {
                    ...
                    for (let i = 0; i < images.length; i++) {
                        ...
                        if (!imgURL.includes('/expression/') && !imgURL.includes('//tvax')) {
