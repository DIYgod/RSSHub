...
const { fallback, queryToToBoolean, queryToToInteger } = require('@/utils/readable-social');

const weiboUtils = {
    formatExtended: (ctx, status, params = {}, picsPrefixes = []) => {
        const newParams = {
            readable: fallback(params.readable, queryToToBoolean(ctx.query.readable), false),
            authorNameBold: fallback(params.authorNameBold, queryToToBoolean(ctx.query.authorNameBold), false),
            showAuthorInTitle: fallback(params.showAuthorInTitle, queryToToBoolean(ctx.query.showAuthorInTitle), false),
            showAuthorInDesc: fallback(params.showAuthorInDesc, queryToToBoolean(ctx.query.showAuthorInDesc), false),
            showAuthorAvatarInDesc: fallback(params.showAuthorAvatarInDesc, queryToToBoolean(ctx.query.showAuthorAvatarInDesc), false),
            showAtBeforeAuthor: fallback(params.showAtBeforeAuthor, queryToToBoolean(ctx.query.showAtBeforeAuthor), false),
            showEmojiForRetweet: fallback(params.showEmojiForRetweet, queryToToBoolean(ctx.query.showEmojiForRetweet), false),
            showRetweetTextInTitle: fallback(params.showRetweetTextInTitle, queryToToBoolean(ctx.query.showRetweetTextInTitle), true),
            addLinkForPics: fallback(params.addLinkForPics, queryToToBoolean(ctx.query.addLinkForPics), false),
            showTimestampInDescription: fallback(params.showTimestampInDescription, queryToToBoolean(ctx.query.showTimestampInDescription), false),

            heightOfPics: fallback(params.heightOfPics, queryToToInteger(ctx.query.heightOfPics), -1),
            heightOfAuthorAvatar: fallback(params.heightOfAuthorAvatar, queryToToInteger(ctx.query.heightOfAuthorAvatar), 48),
        };
