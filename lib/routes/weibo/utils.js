...
const weiboUtils = {
    ...
    getVideo: (link) =>
        new Promise(async function(resolve) {
            const videoResponse = await got.post('https://guoxingjun.com/xhb/wb/url', {
                json: true,
                data: {
                    url: link,
                },
            });
            resolve(videoResponse.data);
        }),
    formatVideo: (itemDesc, videoData) => {
        const video = `<br><video src="${videoData.play_url}"  controls="controls" poster="${videoData.img_url}" style="width: 100%"></video>`;
        return itemDesc + video;
    },
};
