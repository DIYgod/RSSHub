...
const weiboUtils = {
    format: (status) => {
        return weiboUtils.formatWithRetweet(status).description;
    },
    formatWithRetweet: (status, showAuthorAvatar = true) => {
        ...
        if (status.retweeted_status) {
            ...
            temp += retweetedOrOriginal + `<br><small>原博：<a href="https://weibo.com/${status.retweeted_status.user.id}/${status.retweeted_status.bid}" target="_blank" rel="noopener noreferrer">https://weibo.com/${status.retweeted_status.user.id}/${status.retweeted_status.bid}</a></small><br><small>` + new Date(status.retweeted_status.created_at).toLocaleString() + '</small><br clear="both" /><div style="clear: both"></div></blockquote>';
        }
