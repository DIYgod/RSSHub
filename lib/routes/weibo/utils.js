...
const weiboUtils = {
    formatExtended: (ctx, status, params = {}, picsPrefixes = []) => {
        const newParams = {
            ...
            widthOfPics: fallback(params.widthOfPics, queryToInteger(ctx.query.widthOfPics), -1),
            heightOfPics: fallback(params.heightOfPics, queryToInteger(ctx.query.heightOfPics), -1),
            sizeOfAuthorAvatar: fallback(params.sizeOfAuthorAvatar, queryToInteger(ctx.query.sizeOfAuthorAvatar), 48),
        };
        ...
        if (showAuthorInDesc) {
            ...
            if (authorNameBold) {
                usernameAndAvatar += `<strong>${name}</strong></a>:&ensp;&ensp;`;
            } else {
                usernameAndAvatar += `${name}</a>:&ensp;&ensp;`;
            }
