    ...
    switch (category) {
        case 'user': {
            ...
            if (Number.isNaN(nameOrId)) {
                username = nameOrId;
                id = await cache.tryGet(`instagram:getIdByUsername:${username}`, () => ig.user.getIdByUsername(username) as unknown as Promise<string>, 31_536_000); // 1 year since it will never change
