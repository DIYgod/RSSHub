        ...
        post.map(
            async (item) =>
                await ctx.cache.tryGet(item.link, async () => {
                    ...
                    const data = response.data;
                    const arr = data.match(/"url":"https:\\u002F\\u002Fstatic01.nyt.com\\u002Fimages(\S*),"name":"articleLarge"/g);
                    const imageLink = arr.map((e) => JSON.parse(`{${e}}`).url);

                    const $ = cheerio.load(data);
                    $('.css-pncxxs.etfikam0').remove();
                    $('.css-1xdhyk6.erfvjey0').each((i, e) => {
                        const link = imageLink[i];
                        $(e).replaceWith(`<img src="${link}">`);
                    });
