...
module.exports = async (ctx) => {
    ...
    function getDate(datestr, mouthstr) {
        ...
        const mouthlength = mouthstr.indexOf('月');
        const month = mouthlength > 1 ? mouthstr.substring(0, mouthlength) : '0' + mouthstr.substring(0, mouthlength);
    ...
    ctx.state.data = {
        ...
        description: '微语简报',
        item: data.map((item) => ({
            title: item[0].title.replace('【微语简报】', ''),
