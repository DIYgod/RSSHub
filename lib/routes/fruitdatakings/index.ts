...
const handler = async (ctx) => {
    ...
    const items: DataItem[] = data.slice(0, limit).map((item) => {
        const { details } = item;
        const articleLink = `${baseUrl}/china/${product.toLowerCase()}/`;

        const description = `
                <p><strong>${details.variety}</strong> from <strong>${details.origin}</strong></p>
                <p>Retailer: ${details.retailer}</p>
                <p>Price: ${details.price} per ${details.pack} (${details.weight}g)</p>
                ${details.is_bio ? '<p>Organic (Bio)</p>' : ''}
                <img src="${item.image_url}" alt="${details.variety} price photo" />
            `;
