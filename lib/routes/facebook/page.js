...
const fetchCookie = (cache) => {
    const cacheKey = 'facebook-cookie';

    cache.tryGet(cacheKey, async () => {
...
const fetchPageHtml = (linkPath, cacheKey, cache, cookie) => {
...
const parseStoryPage = async (linkPath, cache, cookie) => {
    ...
    const html = await fetchPageHtml(linkPath, cacheKey, cache, cookie);
    ...
    const $content = $box.find('div.ce > div').eq(0);
    ...
    const $attach = $story.find('div.cf').eq(0);
...
module.exports = async (ctx) => {
    ...
    const cookie = await fetchCookie(ctx.cache);
    const html = await fetchPageHtml(linkPath, pageId, ctx.cache, cookie);
    const $ = cheerio.load(html);
    ...
    const items = await Promise.all(
        itemLinks.map(async (itemLink) => {
            if (/^\/.+\/photos\//.test(itemLink)) {
                const data = await parsePhotoPage(itemLink, ctx.cache, cookie);
                return {
                    title: data.title,
                    link: data.url,
