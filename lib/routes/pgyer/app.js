...
=======
                    if (item === val) {
>>>>>>> 57431346bd42fb0e832a08c92d85380783da8505
                        pageTemplate = val;
                    }
                });
            });
    ...
    const version = $(`${indexOfEle[pageTemplate].version}`)
        .children('li')
        .eq(0)
        .text();
    const appSize = $(`${indexOfEle[pageTemplate].appSize}`)
        .children('li')
        .eq(1)
        .text()
        .slice(3);
        ...
        .reduce(
            (acc, curr) =>
                (acc += `<img src='${$(curr)
                    .children()
                    .find('img')
                    .attr('src')}'  style="margin: 0 auto;">`),
            ''
        );
    let appStatus = $(`${indexOfEle[pageTemplate].appStatus}`)
        .text()
        .trim();
<<<<<<< HEAD
    if (appStatus == '点击安装') {
...
>>>>>>> 57431346bd42fb0e832a08c92d85380783da8505
    ...
    let updateIntro = $(`${indexOfEle[pageTemplate].updateIntro}`)
        .text()
        .trim()
        .slice(4);
    if (updateIntro === '') {
