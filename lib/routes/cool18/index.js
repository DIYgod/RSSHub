    ...
    items = await Promise.all(
        items.map((item) =>
            cache.tryGet(item.link, async () => {
                ...
                const senders = detailResponse.data.match(/送交者: (.*)\[/)
                item.author = senders.length > 1 ? senders[1] : 'no name'
                const dates = detailResponse.data.match(/于 (.*) 已读/)
                const publishDate = dates.length > 1 ? dates[1] : ''
                item.pubDate = publishDate ? parseDate(publishDate).toUTCString() : new Date().toUTCString()
