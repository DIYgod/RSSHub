...
module.exports = async (ctx) => {
    ...
    const items = await Promise.all(
        feed.items.map(async (item) => {
            ...
            const description = $('td[id^="postmessage"]').first();
            const imgNode = $(description).find('ignore_js_op > img');
            if (imgNode.attr('src') && imgNode.attr('src').indexOf('none.gif') !== -1) {
                imgNode.attr('src', imgNode.attr('zoomfile'));
            }
            $(description)
