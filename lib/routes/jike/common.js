        ...
        data.posts.map((item) => {
            ...
            if (item.linkInfo) {
                ...
                if (videoObject) {
                    ...
                    const videoDuration = Math.floor(videoObject.duration / 60_000);
            ...
            if (item.pictures) {
                for (const pic of item.pictures) {
                    if (pic.format === 'gif') {
                        ...
                        const imgUrl = pic.picUrl.match(/\.[\da-z]+?\?imageMogr2/) ? pic.picUrl.split('?imageMogr2/')[0] : pic.picUrl.replace(/thumbnail\/.+/, '');
