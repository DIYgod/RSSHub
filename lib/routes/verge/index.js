...
module.exports = async (ctx) => {
    ...
    const items = await Promise.all(
        feed.items.slice(0, 10).map(async (item) => {
            ...
            if (cover.length > 0) {
                $(`<img referrerpolicy="no-referrer" src=${cover[0].attribs.content}>`).insertBefore(content[0].childNodes[0]);
            }
            ...
            content.find('figure.e-image').each((i, e) => {
                ...
                const figure = `<figure><img referrerpolicy="no-referrer" src=${src}>${caption ? `<br><figcaption>${caption}</figcaption>` : ''}</figure>`;
