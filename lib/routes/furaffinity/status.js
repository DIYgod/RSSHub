...
module.exports = async (ctx) => {
    ...
    if (Object.keys(data)[0] === 'online') {
        description = `Status: ${Object.keys(data)[0]} <br> Guests: ${status.guests} <br>Registered: ${status.registered} <br> Other: ${status.other}  <br>Total: ${status.total} <br> Fa Server Time: ${
            data.fa_server_time
        }  <br> Fa Server Time at: ${data.fa_server_time_at} `;
    } else {
        description = 'offline';
    }
    const item = [];
