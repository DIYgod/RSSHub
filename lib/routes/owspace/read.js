...
const handleItem = (ctx) => async (item) => {
    if (parseInt(item.model) === 4) {
        const result = {
            title: item.title.slice(0, 4) + '年' + parseInt(item.title.slice(4, 6)) + '月' + parseInt(item.title.slice(6, 8)) + '日',
            link: item.html5,
...
module.exports = async (ctx) => {
    const selectedType = ctx.params.type === undefined ? 0 : parseInt(ctx.params.type);
