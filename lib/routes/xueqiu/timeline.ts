...
async function handler(ctx) {
    ...
    const usergroup_id = ctx.req.query('usergroup_id') || 1;
    if (cookie === undefined) {
        ...
        const items = await Promise.all(
            data.home_timeline.map((item) =>
                cache.tryGet(item.target, async () => {
                    const retweetedStatus = (await item.retweeted_status) ? `<blockquote>${item.retweeted_status.user.screen_name}:&nbsp;${item.retweeted_status.description}</blockquote>` : '';
