...
module.exports = async (ctx) => {
    ...
    ctx.state.data = {
        ...
        item: data.map((item) => {
            ...
            const description = `<a href="https://xueqiu.com/u/${item.user.id}"><div style='font-weight: bold;'>${item.user.screen_name}:</div></a><hr/>${item.text}<br/><hr/><div style="text-align:right">----来源于:${item.source}</div>`;
            return {
                title: item.text.replace(/<[^>]+>/g, '').replace('&nbsp', '').slice(0, 100),
