...
const getPage = async (id, typeName) => {
    const response = await got.get(apiUrl + id, {
        headers: {
            Referer: link,
        },
    });
    ...
    return Promise.all(
        response.data.msg.result.map(async (item) => {
            let description = await got.get(`https://apps.game.qq.com/wmp/v3.1/public/searchNews.php?p0=18&source=web_pc&id=${item.iNewsId}`);
