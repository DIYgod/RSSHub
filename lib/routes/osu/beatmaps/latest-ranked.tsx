...
async function handler(ctx): Promise<Data> {
    ...
    const rssItems: DataItem[] = beatmapsetList.map((beatmapset) => {
        ...
        const description = renderToString(
            <>
                <img src={cover} alt={beatmapset.title} style="max-width: 100%; height: auto;" />
                <h3>Song Info</h3>
                <ul>
                    <li>
                        <strong>English Title:</strong> {beatmapset.title}
                    </li>
                    <li>
                        <strong>Artist:</strong> {`${beatmapset.artist_unicode} (${beatmapset.artist})`}
                    </li>
                    <li>
                        <strong>Length:</strong> {readableTotalLength}
                    </li>
                    <li>
                        <strong>BPM:</strong> {beatmapset.bpm}
                    </li>
                </ul>
                <h3>Beatmapset Info</h3>
                <ul>
                    <li>
                        <strong>Mode:</strong> {modeLiteralToDisplayNameMap[beatmapset.beatmaps[0].mode]}
                    </li>
                    <li>
                        <strong>Creator:</strong> {beatmapset.creator}
                    </li>
                </ul>
                <h3>Difficulties</h3>
                <table border="1">
