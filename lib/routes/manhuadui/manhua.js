...
module.exports = async (ctx) => {
    ...
    const comicTitle = $('div.comic_deCon > h1')
        .text().trim();
    const comicDescription = $('p.comic_deCon_d')
        .text().trim();
    if (isNaN(serial) || serial <= 0 || contentLength < serial) {
        var list = $('div.zj_list_con > ul > li > a');
    } else {
        var list = $('div.zj_list_con').eq(serial - 1).find('ul > li > a');
    }
    ctx.state.data = {
        title: '漫画堆 - ' + comicTitle,
        link: comicPage,
        description: comicDescription,
        item: list.map((i, item) => ({
