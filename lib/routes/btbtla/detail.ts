...
function buildItem(row: Row, magnet: string): DataItem {
    return {
        ...
        // Season info for structured consumers reading the raw object; _extra is
        // RSSHub's sanctioned passthrough (ignored by RSS/Atom, kept in JSON Feed).
        _extra: { seasonTitle: row.seasonTitle, seasonUrl: row.seasonUrl },
    };
}
...
async function parseDetail(entry: { href: string; title: string }): Promise<Row[]> {
    ...
    const html = await ofetch(seasonUrl);
    const $ = load(html);
...
async function getEntries(name: string): Promise<Array<{ href: string; title: string }>> {
    const html = await ofetch(`${ROOT_URL}/search/${name}`);
    const $ = load(html);
...
async function getMagnet(downPage: string): Promise<string> {
    try {
        const html = await ofetch(downPage);
        const $ = load(html);
