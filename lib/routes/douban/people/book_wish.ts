...
export const route: Route = {
    ...
    maintainers: ['exherb'],
    handler,
    description: `对于豆瓣用户想读的内容，在 \`routeParams\` 参数中以 query string 格式设置如下选项可以控制输出的样式
...
async function handler(ctx) {
    ...
    for (let page = 0; page < pagesCount; page += 1) {
        const url = `https://book.douban.com/people/${userid}/wish?start=${page * pageSize}`;
    ...
    return {
        title: `豆瓣想读 - ${userName || userid}`,
        link: `https://book.douban.com/people/${userid}/wish`,
        item: items,
    };
}
