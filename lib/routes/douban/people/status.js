        ...
        default:
            if (status.card) {
                ...
                if (status.card.rating) {
                    description += `<a href="${status.card.url}">《${status.card.title}》</a><br>
					${status.text ? `${status.author.name}的评分：${status.text}<br>` : ''}豆瓣评分：${status.card.rating}<br>${status.card.subtitle}${image ? `<br><img src="${image}">` : ''}`;
