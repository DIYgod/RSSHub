...
function getContentByActivity(item, isRenderingRepost = false) {
    ...
    let lastIndex = 0;
    let replacedTextSegements = [];
    for (let entity of status.entities) {
    ...
    if (status.video_card) {
        ...
        description += `${videoSrc ? `<video src="${videoSrc}" ${videoCover ? `poster="${videoCover}"` : ''}></video>` : ''}<br>${
    ...
    if (status.reshared_status) {
        ...
        let reshared_url = status.reshared_status.uri.replace("douban://douban.com", "https://www.douban.com/doubanapp/dispatch?uri=");
