        ...
        feed.items.map(
            async (item) =>
                await ctx.cache.tryGet(item.link, async () => {
                    ...
                    const content = $('.articleBody > div[amp-access="access"]:not([class="media-object-podcast"])');

                    // Cover
                    const cover = $('.articleLead > div.is-lead-inset > div.header > .img-header > div.image-container > amp-img');

                    if (cover.length > 0) {
                        $(`<img src=${cover[0].attribs.src}>`).insertBefore(content[0].childNodes[0]);
                        $(cover).remove();
                    }
