...
module.exports = async (ctx) => {
    ...
    function filterHandler(item) {
        ...
        const filter = item.slice(0, Math.max(0, item.indexOf('=')));
        const option = item.slice(Math.max(0, item.lastIndexOf('=') + 1));
    ...
    if (feeds.search(/feeds?=/g) !== -1 || !isNaN(parseInt(feeds.split('&').join('')))) {
