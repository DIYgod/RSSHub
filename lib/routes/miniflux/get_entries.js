...
module.exports = async (ctx) => {
    ...
    function filterHandler(item) {
        ...
        // for each parameter, so in order to matching the default behavior
        // here we use a status variable to track the setting for `limit`.
        // And please note that option `status` could be repeated,
        // which requires the version of MiniFlux instance >= 2.0.24.
        // To find all available options please visit
        ...
        switch (filter) {
            case 'mark':
                if ((option === 'read' || option === 'removed' || option === 'unread') && !setMark) {
                    mark = option;
                    setMark = 1;
                }
                item = '';
            ...
            //     break;
            case 'feed_name':
                if (parseInt(option) === 1 && !setFeedName) {
                    addFeedName = 1;
                    setFeedName = 1;
                }
                item = '';
    ...
    if (feeds.search(/feed[s]?=/g) !== -1 || !isNaN(parseInt(feeds.split('&').join('')))) {
        ...
        const url = `${instance}/v1/entries?${parameters}`;
        const response = await got({
            method: 'get',
            url: url,
            headers: {
