...
module.exports = async (ctx) => {
    ...
    const list = $('h3')
        ...
        .map((_, item) => {
            ...
            return {
                title: item.text(),
                link: url.resolve(rootUrl, item.parent().get(0).tagName === 'a' ? item.parent().attr('href') : item.find('a').attr('href')),
            };
        })
        .get();
            ...
            async (item) =>
                await ctx.cache.tryGet(item.link, async () => {
                    ...
                    if (content('#body').html() !== null) {
                        ...
                        contentBody = content('#comp-rich-text11').html();
                    }
