...
function entry(ctx, list) {
    const now = list.map((line) =>
        ctx.cache.tryGet(line, async () => {
            ...
            const articleRaw = await got.get(`https://port-tdm-com-mo.translate.goog/c_radio/radio_notice_content.php?id=${id}&_x_tr_sl=zh-CN&_x_tr_tl=zh-TW&_x_tr_hl=zh-TW&_x_tr_pto=ajax,op,elem`);
            ...
            if (year > 2019) {
                ...
                const description = $('div#content_wrap').html();
...
module.exports = async (ctx) => {
    ...
    rss = rss.filter((element) => element !== undefined);
