    ...
    const now = list.map(
        async (line) =>
            await ctx.cache.tryGet(line, async () => {
                ...
                // link
                const [, ham] = line.match(/https:\/\/mobile-tdm-com-mo\.translate\.goog(.*)&_x_tr_sl=zh-CN&_x_tr_tl=zh-TW&_x_tr_hl=zh-TW&_x_tr_pto=ajax/);

                return {
                    title: title,
                    description: content,
                    pubDate: time,
                    link: `https://mobile.tdm.com.mo${ham}`,
                };
            })
    );
...
module.exports = async (ctx) => {
    ...
    const response = await got({
        method: 'get',
        url: 'https://mobile-tdm-com-mo.translate.goog/cn/index_pop.php?se=inews&type=all&_x_tr_sl=zh-CN&_x_tr_tl=zh-TW&_x_tr_hl=zh-TW&_x_tr_pto=ajax',
    });
