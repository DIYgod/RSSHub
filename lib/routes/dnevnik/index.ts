...
const rootUrl = 'https://www.dnevnik.bg';
const feedUrl = 'https://www.dnevnik.bg/rss/';

export const route: Route = {
    path: '/',
    categories: ['traditional-media'],
    example: '/dnevnik',
    radar: [{ source: ['dnevnik.bg/', 'dnevnik.bg/*'], target: '/' }],
    name: 'News',
    maintainers: ['lisyer'],
    url: 'dnevnik.bg/',
    handler,
...
async function handler(ctx) {
    ...
    return {
        title: feed.title || 'Dnevnik',
        link: feed.link || rootUrl,
        description: feed.description,
        language: feed.language || 'bg',
        item: items,
    };
}
