...
const buildDescription = (ad) =>
    ...
    [
        (ad.images?.thumb_url && `<img src="${ad.images.thumb_url}">`) || '',
        '',
        ...ad.body.split(/\n/),
        '',
        ((ad.images?.urls_thumb && ad.images.urls_thumb.map((url, i) => `<a href=${ad.images.urls_large[i]}><img src="${url}"></a>`)) || []).join(''),
    ].join('<br>');
...
export default async (ctx) => {
    ...
    ctx.state.data = {
        ...
        item: (response.data.ads || [])
            ...
            .map((ad) => ({
                title: (ad.price?.[0] ? `${ad.price[0]}€ - ` : '') + ad.subject,
