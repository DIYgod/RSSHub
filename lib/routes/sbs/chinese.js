    ...
    const items = await Promise.all(
        list.map((item) =>
            ctx.cache.tryGet(item.link, async () => {
                ...
                const audio = content('a[download]') ? content('a[download]').attr('href') : '';
                const image = content('meta[itemprop="image"]') ? content('meta[itemprop="image"]').attr('content').split('/crop/')[0] : '';
