...
module.exports = async (ctx) => {
    ...
    const out = [];
    // 服务器禁止单个IP大并发访问，不能使用Promise.all
    // TODO: 可以考虑使用promise-pool限制并发数
    for (let i = 0; i < list.length; i++) {
        /* eslint-disable no-await-in-loop */
        const item = await fetchPage(ctx, list[i]);
        out.push(item);
    }

    ctx.state.data = {
        title: '北极星环保 - 环保行业追至门户网站',
        link: listURL,
        item: out,
    };
};
...
async function fetchPage(ctx, link) {
    ...
    ctx.cache.set(link, JSON.stringify(item), 48 * 3600);
    return item;
}
