        ...
        list.map(
            async (item) =>
                await ctx.cache.tryGet(item.link, async () => {
                    ...
                    content('.gallery-remarks, .overlay-text-v2').remove();

                    item.title = content('h2').eq(0).text();
                    item.description = content('.content').html();
                    item.pubDate = item.pubDate || timezone(parseDate(`${pubDate.substr(6, 4)}-${pubDate.substr(0, 5)} ${pubDate.substr(11, 5)}`, 'YYYY-MM-DD HH:mm'), +8);
