...
module.exports = async (ctx) => {
    ...
    const configstr = ctx.params.config || '';
    let mailConfig = {
        username: email,
        password: '',
        host: '',
    ...
    if (config) {
        mailConfig = Object.assign(mailConfig, queryString.parse(configstr));
    } else {
        mailConfig = Object.assign(mailConfig, queryString.parse(config.email.config[email]));
    }

    if (!mailConfig.username || !mailConfig.password || !mailConfig.host || !mailConfig.port) {
        throw Error('please config email password, host, port in .env file or URL');
    }
