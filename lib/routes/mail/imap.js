...
module.exports = async (ctx) => {
    ...
    const items = await Promise.all(
        mails.reverse().map(async (item) => {
            ...
            let mailstr = item['body[]'];
            const charset = jschardet.detect(mailstr);
            if (!['ascii', 'usascii'].includes(charset.encoding.toLowerCase())) {
                mailstr = iconv.decode(mailstr, charset.encoding.replace(/-/g, '').toLowerCase());
            }
            const description = await ParserMail(mailstr);
            const single = {
