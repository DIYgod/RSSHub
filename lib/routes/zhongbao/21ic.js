...
module.exports = async (ctx) => {
    ...
    const geturl = `https://project.21ic.com/list/${category}/`;  // http://project.21ic.com/list/cid25_cidd75
    const response = await got({
        method: 'get',
        url: `${geturl}`,
        headers: {
            Referer: 'https://project.21ic.com',
        },
    });
    ...
    const resultItem = await Promise.all(
        taskList.map(async (i) => {
            const url = 'https://project.21ic.com' + i;
            const task = {
                        title: ``,
                        description: '',
                        link: `${url}`,
