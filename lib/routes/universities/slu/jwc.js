...
module.exports = async (ctx) => {
    const response = await got({
        method: 'get',
        url: baseurl + 'info/iList.jsp?cat_id=13424',
    });
    ...
    const out = await Promise.all(
        ...
        list.map(async (item) => {
            ...
            if (singleUrl.search('http') === -1) {
                singleUrl = `http://jwc.lixin.edu.cn/` + singleUrl;
            }
    ...
    ctx.state.data = {
        title: `教务处通知-上海立信会计金融学院`,
        link: 'http://jwc.lixin.edu.cn/info/iList.jsp?cat_id=13424',
        description: `教务处通知-上海立信会计金融学院`,
        item: out,
    };
};
