...
module.exports = async (ctx) => {
    const response = await got({
        method: 'get',
        url: baseurl + 'info/iList.jsp?cat_id=13789',
    });
    ...
    const out = await Promise.all(
        ...
        list.map(async (item) => {
            ...
            if (singleUrl.search('http') === -1) {
                singleUrl = `http://xsc.lixin.edu.cn/` + singleUrl;
            }
    ...
    ctx.state.data = {
        title: `学生处-上海立信会计金融学院`,
        link: 'http://xsc.lixin.edu.cn/info/iList.jsp?cat_id=13789',
        description: `学生处-上海立信会计金融学院`,
        item: out,
    };
};
