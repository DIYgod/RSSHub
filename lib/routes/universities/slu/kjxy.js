...
module.exports = async (ctx) => {
    const response = await got({
        method: 'get',
        url: baseurl + 'info/iList.jsp?cat_id=13496',
    });
    ...
    const out = await Promise.all(
        ...
        list.map(async (item) => {
            ...
            if (singleUrl.search('http') === -1) {
                singleUrl = `http://kjxy.lixin.edu.cn/` + singleUrl;
            }
    ...
    ctx.state.data = {
        title: `会计学院-上海立信会计金融学院`,
        link: 'http://kjxy.lixin.edu.cn/info/iList.jsp?cat_id=13496',
        description: `会计学院-上海立信会计金融学院`,
        item: out,
    };
};
