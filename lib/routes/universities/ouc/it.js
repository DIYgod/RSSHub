...
module.exports = async (ctx) => {
    ...
    const dateDict = {};
    const list = $('#wp_news_w33').find('li')
        .slice(0, 9)
        .map((i, e) => {
            const divs = $(e).children();
            tlink = host + divs.find('a')[0].attribs.href.substring(1);
            date = $($(divs.children()[0]).find('p')[0]).text();
            dateDict[tlink] = new Date(date).toUTCString();
            return tlink;
        })
        .get();
