...
module.exports = async (ctx) => {
    const url = `https://zb.caa.edu.cn/bszs/zsxx/index.html`;
    ...
    // ## 获取列表
    const list = $('#news_list_right_count > ul > li > a').get();
    // ## 定义输出的item
    const out = await Promise.all(
        ...
        list.map(async (item) => {
            ...
            singleUrl = 'https://zb.caa.edu.cn/' + singleUrl;
            ...
            const detail_content = $('#news_list_xx_count').html();
            // ### 设置 RSS feed item
            const single = {
                title: title,
                link: singleUrl,
