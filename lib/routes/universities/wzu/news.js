...
module.exports = async (ctx) => {
    const newsTypeTag = parseInt(ctx.params.type) || 0;
    const newsLink = new URL(urlList[newsTypeTag], baseUrl).href;
    const response = await got.get(newsLink);
    const $ = cheerio.load(response.data);
    const list = $('#News-sidebar-b-nav').find('li');

    ctx.state.data = {
        title: newsList[newsTypeTag],
        link: newsLink,
        description: '温州大学' + ' - ' + newsList[newsTypeTag],
        item:
            list &&
            list
                .map(async (item) => {
