...
module.exports = async (ctx) => {
  ...
  ctx.state.data = {
    ...
    item: data.map((item) => ({
      ...
      description: `Length: ${item.duration.split(":").map((i) => [("00" + i).slice(-2)]).join(":")}<br/>Auther: ${item.author}    (ID: ${item.mid})<br/>Play: ${item.play}    Favorite: ${item.favorites}<br/>Review: ${item.video_review}    Comment: ${item.review}<br/>Category: ${item.typename}<br/><br/>${item.description.replace(/\n/g, "<br/>")}<br/><img referrerpolicy="no-referrer" src="${item.pic}"><br/>Tag: ${item.tag}<br/>Match By: ${item.hit_columns.join(",")}`,
