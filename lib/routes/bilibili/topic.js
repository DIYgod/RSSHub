...
export default async (ctx) => {
    ...
    ctx.state.data = {
        ...
        item: data.map((item) => {
            ...
            return {
                title: cardData.title || cardData.description || cardData.content || (cardData.vest?.content),
                description: `${cardData.desc || cardData.description || cardData.content || cardData.summary || (cardData.vest?.content) + (cardData.sketch?.title)}${imgHTML} `,
