...
export const route: Route = {
    path: '/user/collection/:uid/:sid/:disableEmbed?/:page?',
    categories: ['social-media'],
    example: '/bilibili/user/collection/245645656/529166',
    parameters: { uid: '用户 id, 可在 UP 主主页中找到', sid: '合集 id, 可在合集页面的 URL 中找到', disableEmbed: '默认为开启内嵌视频, 任意值为关闭', page: '页码, 默认1' },
    features: {
        requireConfig: false,
...
async function handler(ctx) {
    ...
    const host = `https://api.bilibili.com/x/polymer/space/seasons_archives_list?mid=${uid}&season_id=${sid}&sort_reverse=false&page_num=${page}&page_size=${limit}`;
