...
module.exports = async (ctx) => {
    ...
    ctx.state.data = {
        ...
        item: list.map((item) => ({
            ...
            pubDate: data.create && new Date(data.create).toUTCString(),
            author: item.author,
            link: !data.create || (new Date(data.create) / 1000 > utils.bvidTime && item.bvid) ? `https://www.bilibili.com/video/${item.bvid}` : `https://www.bilibili.com/video/av${item.aid}`,
        })),
    };
};
