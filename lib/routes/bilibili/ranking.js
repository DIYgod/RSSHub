...
module.exports = async (ctx) => {
    ...
    list = list.map((i) => i.aid).slice(0, 100);
    const detailsResponse = await got({
        method: 'get',
        url: `https://api.bilibili.com/x/article/archives?ids=${list.join(',')}`,
    });
    ctx.state.data = {
        title: `bilibili ${day}日排行榜-${rid_type}-${arc_type1}`,
        link: `https://www.bilibili.com/ranking/all/${rid}/0/${day}`,
        item: list.map((aid) => {
            const data = detailsResponse.data.data[aid];
            if (data) {
                return {
                    title: data.title,
                    description: `${data.desc}${!disableEmbed ? `<br><br>${utils.iframe(aid)}` : ''}<br><img src="${data.pic}">`,
