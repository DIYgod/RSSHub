...
module.exports = async (ctx) => {
    ...
    const list_rid = ['0','1','168','3','129','4','36','160','119','155','5','181']
    const list_rid1 = ["全站","动画","国创相关","音乐","舞蹈","游戏","科技","生活","鬼畜","时尚","娱乐","影视"]
    const list_i= list_rid.indexOf(rid)
    const list_type=list_rid1[list_i]
    ...
    ctx.state.data = {
        title: `bilibili ${day}日排行榜-${list_type}-${arc_type1}`,
        link: `https://www.bilibili.com/ranking/all/${rid}/0/${day}`,
        item: list.map((item) => ({
            title: item.title,
            description: `作者：${item.author}<br>播放量：${item.play}<br>时长：${item.duration}<br>综合得分：${item.pts}<br>${(item.play/10000).toFixed(1)}万播放<br>${(item.video_review/10000).toFixed(1)}万弹幕<br><img referrerpolicy="no-referrer" src="${item.pic}">`,
