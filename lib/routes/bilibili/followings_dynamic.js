...
module.exports = async (ctx) => {
    ...
    ctx.state.data = {
        ...
        item: data.map((item) => {
            ...
            //番剧标题
            const bangumi_title = parsed.apiSeasonInfo || parsed;
            ...
            const getTitle = (data) => bangumi_title.title || data.title || data.description || data.content || (data.vest && data.vest.content) || '';
            const getDes = (data) =>
            ...
            if (item.display){} else if (item.display.emoji_info) {
            ...
            return {
                title: getTitle(data),
                author : author,
                description: `${parse.new_desc || data_content || getDes(data)}${
