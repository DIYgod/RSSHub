    ...
    ctx.state.data = {
        ...
        item: data.map((item) => {
            ...
            const getOriginDes = (data) =>
                (data && data.apiSeasonInfo && data.apiSeasonInfo.title && `转发自: ${data.apiSeasonInfo.title}`) ||
                '';
            ...
            return {
                ...
                description: `${getDes(data)}${
                    origin && getOriginName(origin) ?
                        `<br><br>转发自: @${getOriginName(origin)}: ${getOriginTitle(origin.item || origin)}${getDes(origin.item || origin)}` :
                        `${getOriginDes(origin)}`
