...
module.exports = async (ctx) => {
    ...
    ctx.state.data = {
        ...
        image: String(data[0].desc.user_profile.info.face),
        item: data.map((item) => {
            const getTitle = (data) => (data ? data.title || data.description || data.content || (data.vest && data.vest.content) || '' : '').substr(0, 25);
            const getDes = (data) =>
