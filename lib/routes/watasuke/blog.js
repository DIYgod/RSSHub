...
export default async (ctx) => {
    ...
    const itemsPromises = articles.map(async (article) => {
        ...
        const ogImage = $article('meta[property="og:image"]').attr('content') || 'https://watasuke.net/static/b549c082da1248181185ec5d6ffe0a69/thumbnail.jpg';

        return {
            title: article.title,
            description: ogDescription,
            pubDate: parseDate(article.published_at),
            updated: parseDate(article.updated_at),
            link: articleUrl,
            category: article.tags.map((tag) => tag.name),
            itunes_item_image: ogImage,
        };
    });
