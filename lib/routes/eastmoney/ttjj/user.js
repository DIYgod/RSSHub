...
module.exports = async (ctx) => {
    ...
    const result = await Promise.all(
        data.map(async (item) => {
            const cacheVal = await ctx.cache.get(`${userArticleUrl}${item.post_id}`);
            if (!cacheVal) {
                ...
                const single = {
                    ...
                    pubDate: parseDate(item.post_display_time, 'YYYY-MM-DD HH:mm:ss'),
                    link: `https://fundbarmob.eastmoney.com/index.html?goPage=articleView&lastPage=personDetailView&aid=${item.post_id}`,
                };
                ctx.cache.set(`${userArticleUrl}${item.post_id}`, JSON.stringify(single));
                return single;
            } else {
