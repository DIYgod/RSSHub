...
module.exports = async (ctx) => {
    ...
    const host = 'https://www.shuquge.com';
    // https://www.shuquge.com/txt/8659/
    const url_t = `ttps://www.shuquge.com/txt/${uid}/`;
    ...
    if (newOneDate.search('月') !== -1 && newOneDate.search('號') !== -1) {
    ...
    if (finished_text.search('已完結') !== -1 || finished_text.search('已完结') !== -1) {
        finished = true;
    }
    // 最新一话的地址
    ...
    const items = list
        .map((index, item) => {
            ...
            if (itemUrl === updatedOne) {
                itemDate = newOneDate.toUTCString();
            } else {
                itemDate = new Date(newOneDate.getFullYear(), newOneDate.getMonth(), newOneDate.getDate() - 7).toUTCString();
            }
            return {
                title: itemTitle,
                link: host + itemUrl,
