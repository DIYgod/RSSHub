...
const ProcessFeed = ({ data = [] }, showAuthor = false, showAuthorInTitle = false) => {
    ...
    return data.map((item) => {
        ...
        return {
            ...
            description: `${showAuthor ? `<a href="https://twitter.com/${item.user.screen_name}"><img align="left" src="${item.user.profile_image_url_https}" hspace="8" vspace="8"><strong>${item.user.name}</strong></a>:&nbsp;&nbsp;` : ''}${item.in_reply_to_screen_name ? '🔁 ' : ''}${
                item.full_text
