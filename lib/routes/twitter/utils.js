...
const ProcessFeed = ({ data = [] }, showAuthor = false, showAuthorInTitle = false) => {
    ...
    const formatText = (text) => text.replace(/https:\/\/t\.co(.*)/g, '').trim().replace(/\n/g, '<br>');
    ...
    return data.map((item) => {
        ...
        return {
            title: originalItem.user.name + `: ${originalItem === item ? '' : '🔁 '}${(showAuthorInTitle || originalItem !== item) ? (item.user.name + ': ') : ''}${item.in_reply_to_screen_name ? '↩️ ' : ''}${replaceBreak(item.full_text)}${quoteOnTitle}`,
            author: originalItem.user.name,
            description: `${showAuthor ? `<a href="https://twitter.com/${item.user.screen_name}"><img align="left" src="${item.user.profile_image_url_https}" hspace="8" vspace="8"><strong>${item.user.name}</strong></a>:&nbsp;&nbsp;` : ''}${item.in_reply_to_screen_name ? 'Re ' : ''}${
                item.full_text
            }${url}${img}${quote}<hr><small>${new Date(item.created_at).toLocaleString()}</small>`,
