...
export const getArticle = (item) =>
    cache.tryGet(item.link, async () => {
        ...
        content
            .find(".appGameBuyCardIframe, .GSAppButton, .Mid2L_down")
            .remove();
        content.find("a").each((_, item) => {
            if (
                item.attribs.href?.startsWith(
                    "https://www.gamersky.com/showimage/id_gamersky.shtml?",
                )
            ) {
                item.attribs.href = item.attribs.href.replace(
                    "https://www.gamersky.com/showimage/id_gamersky.shtml?",
                    "",
                );
            }
        ...
        content.find("img").each((_, item) => {
            if (
                item.attribs.src ===
                "http://image.gamersky.com/webimg13/zhuanti/common/blank.png"
            ) {
