...
function desc(item, option) {
    if (option === 'brief') {
        ...
        return `<p><b><a href="http://cablenews.i-cable.com/ci/news/article/${item.vod_id}/${item.id}">${item.title}</a></b><br><br>${btext}<br><br>(${item.created_at})</p>`;
    } else {
...
module.exports = async (ctx) => {
    ...
    // console.log(api_resp.map(a => a.id));
};
