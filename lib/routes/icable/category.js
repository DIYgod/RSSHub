...
function desc(item, option) {
    if (option === 'brief') {
        return `<p>#${item.group_name} (${item.created_at})</p><p><a href="https://ott-news.i-cable.com/zh-hk/l/open-content?cid=${item.id}&ct=1">在 app 打開</a>｜<a href="http://cablenews.i-cable.com/ci/videopage/news/${item.id}">閱讀全文</a></p>`;
