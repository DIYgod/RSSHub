...
module.exports = async (ctx) => {
    ...
    const eraseQuote = str => 
        str.replace(/\[quote\](.+?)\[\/quote\]/, (m, b) => b)
           .replace(/\[b\].*?\[uid=\d+\](.+?)\[\/uid\].+?\[\/b\]/, (m, user) => `@${user} `);
    ...
    const items = $("#m_posts_c").children().filter('table').map((ind, post_) => {
        const post = $(post_);
        ...
        return {
            title, link, description, pubDate, guid: postId
        };
    });
