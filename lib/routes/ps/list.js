...
export default async (ctx) => {
    ...
    const items = data.included?.filter((item, index) => index % 2);
    const sub = data.included?.filter((item, index) => !(index % 2));
        ...
        item:
            items?.map((item, index) => ({
                ...
                description: `大小：${item.attributes['file-size'].value}${item.attributes['file-size'].unit}<br><br>设备：${item.attributes.platforms.join(', ')}<br><br>类型：${item.attributes.genres.join(', ')}<br><br>评分：${
                    ...
                    item.attributes['media-list']?.preview && item.attributes['media-list'].preview[0] && item.attributes['media-list'].preview[0].url
