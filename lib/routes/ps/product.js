...
module.exports = async (ctx) => {
    ...
    // 繁体
    if (lang.indexOf('hant-hk')) {
        langurl = `ch/HK`;
    }
    if (lang.indexOf('hant-tw')) {
        langurl = `ch/TW`;
    }
    ...
    ctx.state.data = {
        ...
        link: `https://store.playstation.com/zh-hans-hk/${gridName}`,
        item: item.map((item) => {
            let price = '';
            let discount = '';
            // 折扣，另外这是选择带有特殊字符的JSON数据方法！
            let npdiscount = '';
            if (item.attributes['badge-info']['non-plus-user'] && item.attributes['badge-info']['non-plus-user']['discount-percentage'] === 100) {
                npdiscount = `${textshow.tprice}:${item.attributes.skus[0].prices['non-plus-user']['actual-price'].display}`;
            } else if (item.attributes['badge-info']['non-plus-user'] && item.attributes['badge-info']['non-plus-user']['discount-percentage'] === 0) {
                npdiscount = `${textshow.tprice}:${item.attributes.skus[0].prices['non-plus-user']['actual-price'].display}`;
            } else if (item.attributes['badge-info']['non-plus-user']) {
                npdiscount = `${textshow.tdiscount} ${item.attributes['badge-info']['non-plus-user']['discount-percentage']}%`;
            }
            // PLUS折扣
            let pdiscount = '';
            if (item.attributes['badge-info']['plus-user'] && item.attributes['badge-info']['plus-user']['discount-percentage'] === 100) {
                pdiscount = `${textshow.tplusfree}`;
            } else if (item.attributes['badge-info']['plus-user'] && item.attributes['badge-info']['plus-user']['discount-percentage'] === 0) {
                pdiscount = `${textshow.tplusfree}`;
            } else if (item.attributes['badge-info']['plus-user']) {
                pdiscount = `PLUS ${textshow.tdiscount} ${item.attributes['badge-info']['plus-user']['discount-percentage']}%`;
            }
            // 价格
            const npprice = `${textshow.tprice}：${item.attributes.skus[0].prices['non-plus-user']['actual-price'].display}`;
            const pprice = `PLUS${textshow.tprice}：${item.attributes.skus[0].prices['plus-user']['actual-price'].display}`;
            // 优惠时限&判断
            let endtime = ``;
            ...
            if (item.attributes['badge-info']['non-plus-user']) {
                price = `${npprice} | ${pprice}<br><br>${endtime}`;
                discount = `|${npdiscount} | ${pdiscount}`;
            } else {
                price = `${npprice} | ${pprice}`;
            }
            return {
                title: `${item.attributes.name}${discount}`,
                description: `${price}<br>${textshow.treleasedate}：${dayjs(item.attributes['release-date']).format(textshow.releasedateformat)}<br><br>${textshow.tsize}：${item.attributes['file-size'].value}${
                    item.attributes['file-size'].unit
                }<br><br>${textshow.tdevice}：${item.attributes.platforms.join(', ')}<br><br>${textshow.ttype}：${item.attributes.genres.join(', ')}<br><br>{textshow.trating}：${item.attributes['star-rating'].score}<br><br>${
                    textshow.tdescribe
                }：${item.attributes['long-description']}<br><br>${textshow.tpreview}：${
                ...
                link: `https://store.playstation.com/zh-hans-hk/product/${gridName}`,
            };
        }),
    };
};
