        ...
        item: item.map((item) => {
            ...
            return {
                ...
                description: `${price}<br>${textshow.treleasedate}：${dayjs(item.attributes['release-date']).format(textshow.releasedateformat)}<br><br>
					...
					${textshow.tpreview}：${item.attributes['media-list'].preview[0].url ? `<video src="${item.attributes['media-list'].preview[0].url}" controls="controls" style="width: 100%"></video>` : textshow.tnopreview}`,
                link: `https://store.playstation.com/${ctx.params.lang}/product/${gridName}`,
            };
        }),
    };
