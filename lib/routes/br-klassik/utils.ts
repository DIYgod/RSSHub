...
export const getArticle = (link: string) =>
    cache.tryGet(link, async () => {
        ...
        $article.find('.br-audio, .br-textbox, .br-social-footer, .br-comments, .br-footer, script, style, .br-info, .br-credits').remove();
        $article
            .find('p')
            .filter((_, elem) => {
        ...
        const pubDate = dateMatch ? parseDate(`${dateMatch[3]}-${dateMatch[2]}-${dateMatch[1]}`) : undefined;
