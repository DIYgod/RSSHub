...
module.exports = async (ctx) => {
    ...
    const items = notifications.map((item) => {
        let link = item.subject.url ? item.subject.url.replace('https://api.github.com/repos/', 'https://github.com/') : 'https://github.com/notifications';
        if (link.includes('/releases/')) {
            link = link.replace(/\/releases\/\d+$/, '/releases');
        }  //Rewrite the API URL to original URL
        return {
