...
module.exports = async (ctx) => {
    ...
    const items = notifications.map((item) => {
        let origin_url = item.subject.url ? item.subject.url.replace('https://api.github.com/repos/', 'https://github.com/') : 'https://github.com/notifications';
        if (origin_url.includes('/releases/')) {
            origin_url = origin_url.replace(/\/releases\/\d+$/, '/releases');
        }
        return {
