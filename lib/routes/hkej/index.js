...
module.exports = async (ctx) => {
    ...
    dayjs.extend(utc);
    const yesterday = dayjs().utcOffset(8, true).subtract(1, 'day').format('YYYY-MM-DD');
    const today = dayjs().utcOffset(8, true).format('YYYY-MM-DD');
            ...
            list.map((item) =>
                ctx.cache.tryGet(item.link, async () => {
                    ...
                    content('div.hkej_detail_thumb_2014 a').each((_, e) => {
                        e = $(e);
                        if (e.attr('href')) {
                            content(e).after(`<figure><img src="${e.attr('href')}" alt="${e.attr('title')}"><figcaption>${e.attr('title')}</figcaption></figure>`);
                            content(e).remove();
                        }
                    });
