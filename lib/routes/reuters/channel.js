...
module.exports = async (ctx) => {
    ...
    if (site === 'us') {
        items = await Promise.all(links.map(async (link) => await ctx.cache.tryGet(link, async () => await utilsUS.ProcessFeedUS(link))));
    }
    else {
        items = await Promise.all(links.map(async (link) => await ctx.cache.tryGet(link, async () => await utils.ProcessFeed(link))));
    }
