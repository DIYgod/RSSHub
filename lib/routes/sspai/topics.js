    ...
    const items = await Promise.all(
        data.map(async (item) => {
            ...
            if (value) {
                ...
                description = `${item.intro}<br><img src="https://cdn.sspai.com/${item.banner}" referrerpolicy="no-referrer" /><br>如有兴趣,请复制链接订阅 <br> <h3>https://rsshub.app/sspai/topic/${item.id}</h3>`;
