...
module.exports = async (ctx) => {
    ...
    let lastEpisodeId;
    const rssData = await axios.get(`https://prod-api.ishuhui.com/ver/67715120/anime/detail?id=${id}&type=comics&.json`)
    ...
    let description = '';
    const data = await axios.get(`https://prod-api.ishuhui.com/comics/detail?id=${lastEpisodeId}`)
        .then(function (response) {
            ...
            for (let i = 0; i < picUrlArray.length; i++) {
                description += `<img referrerpolicy="no-referrer" src="${picUrlArray[i].url}"><br>`;
            }
