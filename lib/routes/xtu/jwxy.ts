...
const rootUrl = 'https://jwxy.xtu.edu.cn/';
const host = 'jwxy.xtu.edu.cn';
...
const handler: Route['handler'] = async (ctx) => {
    ...
    const list = $('.common-list li')
        ...
        .map((item) => {
            ...
            let title = $a.attr('title') || $item.find('p.ellipsis').text() || $a.text() || '';
    ...
    return {
        title: `湘潭大学计算机学院 - ${typeDict[type][0]}`,
        link: listUrl,
        item: items,
    };
};
