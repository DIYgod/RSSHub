        ...
        list.map(
            async (item) =>
                await ctx.cache.tryGet(item.link, async () => {
                    ...
                    item.pubDate =
                        content('meta[property="article:published_time"]').length > 0 ? Date.parse(content('meta[property="article:published_time"]').attr('content')) : parseDate(content('.teaser-date').text(), 'MM/DD/YY');
