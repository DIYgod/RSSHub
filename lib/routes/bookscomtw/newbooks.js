...
module.exports = async (ctx) => {
    ...
    const items = list.map((i, item) => {
        ...
        price = price.length === 1 ? parseInt(price.text().trim()) :
            parseInt(price.eq(0).text() > 10 ? (price.eq(1).text() * 100 / price.eq(0).text()).toFixed() :
                (price.eq(1).text() * 10 / price.eq(0).text()).toFixed());
        return {
            title,
