...
const ProcessFeed = (list) => {
    ...
    return list.map((item) => {
        ...
        if (n1 !== -1) {
            n += parseInt(pubDate.substring(n1 - 3, n1 - 1)) * 365;
        }
        if (n2 !== -1) {
            n += parseInt(pubDate.substring(n2 - 3, n2 - 1)) * 30;
        }
        if (n3 !== -1) {
            n += parseInt(pubDate.substring(n3 - 3, n3 - 1)) * 7;
        }
        if (n4 !== -1) {
            n += parseInt(pubDate.substring(n4 - 3, n4 - 1)) * 1;
        }
        if (n5 !== -1) {
            h += parseInt(pubDate.substring(n5 - 3, n5 - 1)) * 3600;
        }
        if (n6 !== -1) {
            h += parseInt(pubDate.substring(n6 - 3, n6 - 1)) * 60;
        }
        const now = new Date();
