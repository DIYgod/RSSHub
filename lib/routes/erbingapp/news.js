...
module.exports = async (ctx) => {
    ...
    ctx.state.data = {
        ...
        item: data.map((item) => {
            ...
            } else if (item.acontent.toString().indexOf('104129') !== -1) {
                description += `<br><img src="${item.article.humb.split('?')[0]}">`;
            } else if (item.acontent.toString().indexOf('.html') !== -1) {
                description += `<br><a href="${item.acontent}">More</a>`;
            } else if (item.acontent.indexOf('"imgs') !== -1) {
                const imgs = JSON.parse(item.acontent).imgs.split(',');
                for (let i = 0; i < imgs.length; i++) {
                    description += `<br><img src="${imgs[i]}">`;
                }
            }

                const imgs = item.acontent.toString().split(',');
