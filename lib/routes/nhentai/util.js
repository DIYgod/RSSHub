...
const MAX_DETAIL = 5;
exports.getDetails = async (cache, simples) => Promise.all(simples.slice(0, MAX_DETAIL).map((simple) => cache.tryGet(simple.link, () => getDetail(simple))));
...
exports.getTorrents = async (cache, simples) => {
    ...
    return await getTorrentWithCookie(cache, simples, cookie);
};
const getTorrentWithCookie = async (cache, simples, cookie) => Promise.all(simples.slice(0, MAX_TORRENT).map((simple) => cache.tryGet(simple.link + 'download', () => getTorrent(simple, cookie))));
