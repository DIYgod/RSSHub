...
const got = require('@/utils/got').cloudflare;

module.exports = async (ctx) => {
    const { time, name } = ctx.params;
    const $ = await got.get(`https://anime1.me/category/${encodeURIComponent(time)}/${encodeURIComponent(name)}`).then((r) => cheerio.load(r));
