...
module.exports = async (ctx) => {
    ...
    ctx.state.data = {
        ...
        item: await Promise.all(
            $list('.news_box .list li:not(.line)').slice(1, 12)
                .map(async (_, el) => {
                    const $el = $list(el);
                    ...
                    if (value) {
                        ...
                        const contentData = await axios.get(contentUrl);
                        ...
                        if (contentData.data.indexOf('UCAP-CONTENT') != -1){
                            dataEncoode = $content('#UCAP-CONTENT').html();
                        } else{
                            dataEncoode = $content('body').html();
                        }
                        dataDecode = unescape(dataEncoode.replace(/&#x/g,'%u').replace(/;/g,''))
