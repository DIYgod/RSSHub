...
async function load(link, ctx) {
    let { title, pubDate, author } = '';
    const description = await ctx.cache.tryGet(link, async () => {
        // 开始加载页面
        const response = await got.get(link);
        const $ = cheerio.load(response.data);
        // 获取标题
        title = $('body > div.container.mt40 > div > div > div.m-lg.text-center > div.p-sm').text();
        // 获取正文内容
        ...
        pubDate = new Date(new Date(dateTime).getTime()).toUTCString();
        // 获取标题中的作者
        author = temp.substring(temp.indexOf('信息来源') + 5, temp.indexOf('阅读次数')).trim();
