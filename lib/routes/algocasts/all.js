...
const getVideoPubDate = async (ctx, episode) => {
    const cacheKey = `video-publish-date-${episode}`;
    let time = await getCacheParsed(ctx, cacheKey);

    if (time) {
        time = new Date(time);
    } else {
        time = new Date();
        ctx.cache.set(cacheKey, time, 365 * 24 * 60 * 60);
    }
...
const generateResponse = async (ctx, items) => ({
    ...
    item: await Promise.all(
        items.map(async (item) => ({
            ...
            // 文章发布时间
            pubDate: await getVideoPubDate(ctx, item.episode),
            // 文章链接
            link: item.link,
        }))
    ),
...
const makeFull = async (ctx, infos) => {
    ...
    return await Promise.all(
        infos.map(async (item) => {
            ...
            ctx.cache.set(cacheKey, info, 365 * 24 * 60 * 60);
...
module.exports = async (ctx) => {
    const full = Boolean(ctx.request.query.full || false);
    ...
    ctx.cache.set(`last-result`, infos, 30 * 60);
    ctx.cache.set(`last-refresh-time`, new Date(), 30 * 60);
