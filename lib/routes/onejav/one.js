...
module.exports = async (ctx) => {
    ...
    let link = '';
    if (type === 'new') {
        link = `https://onejav.com/${type}`;
    } else if (type === 'day') {
        link = `https://onejav.com/${key}`;
    } else {
        link = `https://onejav.com/${type}/${key}`;
    }
            ...
            list
                .map((index, item) => {
                    ...
                    return {
                        ...
                        description: `<img src="${itemPicUrl}"/><br/>时间：${item.find('p.subtitle.is-6 > a').text()}<br/>介绍：${item.find('p.level.has-text-grey-dark').text()}`,
