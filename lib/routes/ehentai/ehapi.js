...
async function parsePage(cache, data, get_bittorrent = false) {
    ...
    function parseTableRow(cache, element) {
        const el = $(element);
        ...
        const thumbnail = el_img.attr('data-src') || el_img.attr('src');
        ...
        if (title && link) {
            ...
            if (get_bittorrent) {
                ...
                if (bittorrent_page_url) {
                    const bittorrent_url = getBittorrent(cache, bittorrent_page_url);
                    if (bittorrent_url) {
...
async function getFavoritesItems(cache, page, favcat, inline_set, get_bittorrent = false) {
    const response = ehgot(`favorites.php?favcat=${favcat}&inline_set=${inline_set}`);
    if (parseInt(page) === 0) {
        const items = await parsePage(cache, response.data, get_bittorrent);
        return updateBittorrent_url(cache, items);
    }
    return await gatherItemsByPage(cache, `favorites.php?page=${page}&favcat=${favcat}`, get_bittorrent);
}

async function getSearchItems(cache, params, page = undefined, get_bittorrent = false) {
    if (page) {
        return await gatherItemsByPage(cache, `?page=${page}&${params}`, get_bittorrent);
    } else {
        return await gatherItemsByPage(cache, `?${params}`, get_bittorrent);
    }
}

async function getTagItems(cache, tag, page, get_bittorrent = false) {
    return await gatherItemsByPage(cache, `tag/${tag}/${page}`, get_bittorrent);
}

export default { getFavoritesItems, getSearchItems, getTagItems };
