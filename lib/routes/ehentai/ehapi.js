...
async function parsePage(cache, data) {
...
async function gatherItemsByPage(cache, urls) {
    const items_Promises = [];
    for (const url of urls) {
        items_Promises.push((async (url) => await parsePage(cache, (await ehgot(url)).data))(url));
    }
...
async function getFavoritesItems(cache, page, favcat, inline_set) {
    const response = await ehgot(`favorites.php?favcat=${favcat}&inline_set=${inline_set}`);
    let items = await parsePage(cache, response.data);
    if (page !== 0) {
        const urls = [];
        for (let i = 1; i <= page; i++) {
            urls.push(`favorites.php?page=${i}&favcat=${favcat}`);
        }
        items = items.concat(await gatherItemsByPage(cache, urls));
    }
    return updateBittorrent_url(cache, items);
}

async function getSearchItems(cache, params, page = undefined) {
    const urls = [`?${params}`];
    for (let i = 1; i <= page; i++) {
        urls.push(`?page=${i}&${params}`);
    }
    return await gatherItemsByPage(cache, urls);
}

async function getTagItems(cache, tag, page) {
    const urls = [`tag/${tag}`];
    for (let i = 1; i <= page; i++) {
        urls.push(`tag/${tag}/${i}`);
    }
