...
module.exports = async (ctx) => {
 ...
 if (ctx.params.limit === '25') {
    ...
    const list = $('div.item').slice(0, ctx.params.limit).get();
    ...
    const out = await Promise.all(
        list.map(async (item) => {
            ...
            const contents = capture('div.icontent').html();
    ...
    const response = await got.get(host);
    const $ = cheerio.load(response.data);
    const list = $('div.ui-mod-picsummary').get();
    ...
    const out = await Promise.all(
        list.map(async (item) => {
          ...
          const cache = await ctx.cache.get(itemUrl);
            ...
            const res = await got.get(itemUrl);
            const capture = cheerio.load(res.data);
            const contents = capture('div.add-cont-m').html();
