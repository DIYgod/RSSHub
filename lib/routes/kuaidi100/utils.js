...
async function getCompanyList(ctx) {
    const key = 'kuaidi100-company-name';
...
async function shouldUpdateCookie(ctx) {
    ...
    */
    ...
    getQuery: async (ctx, number, id, phone) => {
        ...
        if (!query) {
            ...
            let cache_time = 14400; // Cache 4 hours
            // I would cache result even on error
            query = queryResponse.data;
            if (query.ischeck === '1') {
                cache_time = 31536000; // 1 Year for checked id
            }
            ctx.cache.set(query_key, query, cache_time);
        } else {
