...
module.exports = async (ctx) => {
    ...
    const response = await got({
        method: 'get',
        url: host + `quanzhi?city=${encodeURI(city)}&query=${encodeURI(keyword)}`,
    });
    // 获取页面的html
    ...
    console.log(data);
    // // 抽取主要的html
    // const simpleHtml = $('body > div:nth-child(1) > div > div.inner.home-content > div.bigblock.clearfix > div.listblock > div.listwraper').get();
    // // 将抽取出来的代码作为分析部分
    // // 开始处理数据
    // $ = cheerio.load($(simpleHtml).text());
    // 得到标题List
