...
function ParseContentTypeItem(item) {
    const pubdate = new Date(parseInt(item.content_info.ctime) * 1000 - 8 * 60 * 60 * 1000);
...
const ProcessFeed = async (list, caches) =>
    await Promise.all(
        list.map(async (item) => {
            // 解析时间，补偿时区offset
            const { link, single } = item.article_id ? ParseArticleTypeItem(item) : ParseContentTypeItem(item);
