...
const ProcessFeed = async (list, caches) =>
    await Promise.all(
        list.map(async (item) => {
            // 解析时间，补偿时区offset
            const pubdate = new Date(parseInt(item.item_info.ctime || item.item_info.article_info.ctime) * 1000 - 8 * 60 * 60 * 1000);
            const link = item.item_info.url || `https://juejin.im/post/${item.item_info.article_info.article_id}`;
            // 列表上提取到的信息
            const single = {
                title: item.item_info.title || item.item_info.article_info.title,
                description: `${(item.content || item.summaryInfo || '无描述').replace(/[\x00-\x08\x0b-\x0c\x0e-\x1f\x7f]/g, '')}`,
                pubDate: pubdate.toUTCString(),
                author: item.item_info.author_name || item.item_info.author_user_info.user_name,
                link,
            };
