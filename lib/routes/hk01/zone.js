        ...
        item: list.map((item) => {
            ...
            switch (item.type) {
                case 1:
                    ...
                    description = `<p>${item.data.description}</p><img style="width: 100%" src="https://images.weserv.nl/?url=${item.data.mainImage.cdnUrl}" />`;
                    pubDate = new Date(item.data.lastModifyTime * 1000);
                    break;
                case 2:
                    author = item.data.zonePublishName;
                    description = `${item.data.teaser.map((e) => '<p>' + e + '</p>').join('')}<img style="width: 100%" src="https://images.weserv.nl/?url=${item.data.mainImage.cdnUrl}" />`;
                    pubDate = new Date();
                    break;
                default:
                    break;
