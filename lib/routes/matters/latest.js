...
module.exports = async (ctx) => {
    ...
    const items = await Promise.all(
        response.data.data.viewer.recommendation.feed.edges.map(({ node }) => {
    ...
    for (let i = 0; i < responses.length; i++) {
        ...
        items[i].description = $('.u-content').html();
        ctx.cache.set(items[i].link, JSON.stringify(items[i]), 24 * 60 * 60);
    }
