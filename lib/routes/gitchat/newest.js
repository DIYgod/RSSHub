...
module.exports = async (ctx) => {
    ...
    const selected = ctx.params.selected ? 'true' : 'false';

    const response = await got(`https://gitbook.cn/activities?page=1&type=new&isSelected=${selected}${category ? '&category=' + category : ''}`);
