...
const antiHotlink = require('./anti-hotlink');
...
module.exports = async (ctx, next) => {
    ...
    if (ctx.state.data) {
        ...
        // get anti-hotlink templates, will immediately abort throw an error if any template is invalid
        // help avoid unnecessary load if the templates are invalid
        const antiHotlinkTemplates = antiHotlink.getTemplates(ctx);
        const needAntiHotlinkWorkaroundFlag = Object.values(antiHotlinkTemplates).some(Boolean);

        // only if a parameter can change the media element in item.description, to change the item count, apply
        // anti-hotlink workarounds at last, otherwise in handleItem(), help improve performance in most cases
        // !!! update here if a new parameter is added !!!
        const lazyAntiHotlinkWorkaroundFlag =
            ctx.query &&
            [
                ctx.query.limit && parseInt(ctx.query.limit) < ctx.state.data.item.length,
                // ctx.query.filter_case_sensitive
                ctx.query.filter,
                ...
                ctx.query.filter_time,
                // ctx.query.tgiv,
                ctx.query.mode && ctx.query.mode.toLowerCase() === 'fulltext',
                // ctx.query.scihub,
                // ctx.query.opencc,
                ctx.query.brief,
            ].some(Boolean);
        ...
        const handleItem = (item) => {
            ...
            // handle description
            if (item.description) {
                // disable `isDocument` to get rid of <html><body></body></html>
                const $ = cheerio.load(item.description, undefined, false);
                ...
                if (needAntiHotlinkWorkaroundFlag && !lazyAntiHotlinkWorkaroundFlag) {
                    // apply anti-hotlink workarounds to item.description early
                    item.description = antiHotlink.processHtml($, antiHotlinkTemplates);
                } else {
                    item.description = $.html();
                }
                item.description += '' + (config.suffix || '');
            }
            return item;
        };
        ...
        if (ctx.query) {
            ...
            if (needAntiHotlinkWorkaroundFlag && lazyAntiHotlinkWorkaroundFlag && !ctx.query.brief) {
                // apply anti-hotlink workarounds to item.description if not applied early
                // note that if `brief` enabled, there is no media to be processed
                ctx.state.data.item &&
                    ctx.state.data.item.forEach((item) => {
                        if (item.description) {
                            item.description = antiHotlink.processHtml(item.description, antiHotlinkTemplates);
                        }
                    });
            }

            if (needAntiHotlinkWorkaroundFlag && ctx.state.data.description) {
                ctx.state.data.description = antiHotlink.processHtml(ctx.state.data.description, antiHotlinkTemplates);
            }
        }
    }
};
