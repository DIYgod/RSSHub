...
module.exports = async (ctx, next) => {
    ...
    // 路由为空提醒
    if (ctx.query && ctx.query.warning && ctx.state.data.item.length === 0) {
        ctx.state.data.item = [
            {
                title: '路由内容为空',
                description: '该路由目前获取内容为空，请检查源站情况或前往https://github.com/DIYgod/RSSHub/issues反馈该问题。',
                guid: new Date(new Date().setHours(0, 0, 0, 0)).toUTCString(),
