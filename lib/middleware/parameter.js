...
module.exports = async (ctx, next) => {
    ...
    if (ctx.state.data && ctx.query && ctx.query.mode && ctx.query.mode.toLowerCase() === 'fulltext') {
        const isValidDate = (date) => (!!date && date instanceof Date && date.toString() !== 'Invalid Date' ? date : null);
        const tasks = ctx.state.data.item.map(async (item) => {
            const { link, pubDate, author, description } = item;
            const { body: buffer } = await got(link, {
                responseType: 'buffer',
            });
            ...
            item.pubDate = isValidDate(pubDate) || isValidDate(new Date(result.date_published)) || null;
            item.author = author || result.author;
            item.description = result.content || description;
        });
