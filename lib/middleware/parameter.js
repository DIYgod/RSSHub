        ...
        if (ctx.query) {
            ...
            if (ctx.query.brief) {
                ...
                if (num.test(ctx.query.brief)) {
                    ...
                    ctx.state.data.item.forEach((item) => {
                        item.description = `<p>Invalid parameter <code>brief=${ctx.query.brief}</code>.</p><p>參數 <code>brief=${ctx.query.brief}</code> 不合規範。</p><p>参数 <code>brief=${ctx.query.brief}</code> 不合规范。</p>`;
                    });
                }
            }
        }
