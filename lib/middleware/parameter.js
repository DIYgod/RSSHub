...
module.exports = async (ctx, next) => {
    ...
    if (ctx.state.data) {
        ...
        if (ctx.query) {
            ...
            // brief
            if (/^[1-9][0-9]{3}$/.test(ctx.query.brief)) {
                ctx.query.brief = parseInt(ctx.query.brief);
                ctx.state.data.item.forEach((item) => {
                    let ptext;
                    let btext;
                    if (item.description){
                        ptext = item.description.replace(/<\/?[^>]+(>|$)/g, '');
                    } else {
                        ptext = item.description;
                    }
                    if (ptext && ptext.length && ptext.length > ctx.query.brief) {
                        btext = `${ptext.substring(0, ctx.query.brief)}…`;
                    } else {
                        btext = ptext;
                    }

                    if (item.category) {
                        item.description = `<p>${btext}<br><br>#${item.category} (${item.pubDate})</p>`;
                    } else {
                        item.description = `<p>${btext}<br><br>(${item.pubDate})</p>`;
                    }
                });
            } else {
                return `<p>Invalid parameter <code>brief=${ctx.query.brief}</code>.</p><p>參數 <code>brief=${ctx.query.brief}</code> 不合規範。</p><p>参数 <code>brief=${ctx.query.brief}</code> 不合规范。</p>`;
            }
        }
    }
};
