        ...
        if (ctx.query) {
            ...
            if (ctx.query.filter) {
                ctx.state.data.item = ctx.state.data.item.filter((item) => {
                    ...
                    const isFilter = title.match(makeRegex(ctx.query.filter_title)) || description.match(makeRegex(ctx.query.filter_description)) || author.match(makeRegex(ctx.query.filter_author));
