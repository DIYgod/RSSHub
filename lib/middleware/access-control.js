...
const ipInCidr = (cidr, ip) => {
    ...
    const cidrIpBits = ipv4ToBitsring(cidrMatch[1]).slice(0, Math.max(0, subnetMask));
    const ipBits = ipv4ToBitsring(ip).slice(0, Math.max(0, subnetMask));
...
module.exports = async (ctx, next) => {
    ...
    if (requestPath === '/' || requestPath === '/robots.txt') {
        ...
        if (config.allowlist && config.allowlist.find((item) => ip.includes(item) || ipInCidr(item, ip) || requestPath.includes(item) || requestUA.includes(item))) {
            return grant();
        }

        if (config.denylist && !config.denylist.find((item) => ip.includes(item) || ipInCidr(item, ip) || requestPath.includes(item) || requestUA.includes(item))) {
            return grant();
        }
