const md5 = require('@/utils/md5');
const config = require('@/config').value;
const logger = require('@/utils/logger');
const { RequestInProgressError } = require('@/errors');

const globalCache = {
    get: () => null,
    set: () => null,
};

let get = () => null;
let set = () => null;
let status = { available: false };

if (config.cache.type === 'redis') {
    const { clients, status: redisStatus, get: redisGet, set: redisSet } = require('./redis');
    get = redisGet;
    set = redisSet; // Here we return a Promise
    status = redisStatus;
    const { redisClient } = clients;
    globalCache.get = async (key) => {
        if (key && status.available) {
            const value = await redisClient.get(key);
            return value;
        }
    };
    globalCache.set = set;
} else if (config.cache.type === 'memory') {
    const { clients, status: memoryStatus, get: memoryGet, set: memorySet } = require('./memory');
    get = memoryGet;
    set = memorySet;
    status = memoryStatus;
    const { pageCache } = clients;
    globalCache.get = (key) => {
        if (key && status.available) {
            return pageCache.get(key);
        }
    };
    globalCache.set = (key, value, maxAge) => {
        if (!value || value === 'undefined') {
            value = '';
        }
        if (typeof value === 'object') {
            value = JSON.stringify(value);
        }
        if (key) {
            return pageCache.set(key, value, maxAge * 1000);
        }
    };
} else {
    logger.warning('Cache not available, concurrent requests are not limited. This could lead to bad behavior.');
}

module.exports = function (app) {
    app.context.cache = {
        get,
        set,
        tryGet: async (key, getValueFunc, maxAge = config.cache.contentExpire) => {
            let v = await get(key);
            if (!v) {
                v = await getValueFunc();
                set(key, v, maxAge);
            } else {
                let parsed;
                try {
                    parsed = JSON.parse(v);
                } catch (e) {
                    parsed = null;
                }
                if (parsed) {
                    v = parsed;
                }
            }

            return v;
        },
    };

    return async (ctx, next) => {
        const key = 'koa-redis-cache:' + md5(ctx.request.path);
        const controlKey = 'koa-redis-request:' + md5(ctx.request.path);

        if (!status.available) {
            return next();
        }

        const isRequesting = await globalCache.get(controlKey);

        if (isRequesting) {
            throw new RequestInProgressError('This path is currently fetching, please come back later!');
        }

        try {
            const value = await globalCache.get(key);

            if (value) {
                ctx.response.status = 200;
                if (config.cache.type === 'redis') {
                    ctx.response.set({
                        'X-Koa-Redis-Cache': 'true',
                    });
                } else if (config.cache.type === 'memory') {
                    ctx.response.set({
                        'X-Koa-Memory-Cache': 'true',
                    });
                }
                ctx.state.data = JSON.parse(value);
                return;
            }
        } catch (e) {
            //
        }

        // Doesn't hit the cache? We need to let others know!
        await globalCache.set(controlKey, true, config.cache.requestTimeout);
        await next();

        if (ctx.response.get('Cache-Control') !== 'no-cache' && ctx.state && ctx.state.data) {
            ctx.state.data.lastBuildDate = new Date().toUTCString();
            const body = JSON.stringify(ctx.state.data);
            await globalCache.set(key, body, config.cache.routeExpire);
        }

        // We need to let it go, even no cache set.
        // Wait to set cache so the next request could be handled correctly
        await globalCache.set(controlKey, false, config.cache.routeExpire);
    };
};
