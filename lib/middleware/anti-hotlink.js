...
module.exports = async (ctx, next) => {
    ...
    if (!image_hotlink_template && !ctx.query.multimedia_hotlink_template && !ctx.query.wrap_multimedia_in_iframe) {
        return;
    }
    ...
    if (ctx.state.data) {
        if (ctx.state.data.description) {
            ctx.state.data.description = process(ctx.state.data.description, image_hotlink_template, ctx.query.multimedia_hotlink_template, ctx.query.wrap_multimedia_in_iframe);
        }

        ctx.state.data.item &&
            ctx.state.data.item.forEach((item) => {
                if (item.description) {
                    item.description = process(item.description, image_hotlink_template, ctx.query.multimedia_hotlink_template, ctx.query.wrap_multimedia_in_iframe);
                }
            });
    }
};
