...
const lazyloadRouteHandler = (routeHandlerPath) => (ctx) => {
    let routePath = routeHandlerPath
    if (!routeHandlerPath.endsWith('.js')) {
        routePath = `${routeHandlerPath}.js`
    }

    if (RouterHandlerMap.has(routePath)) {
        return RouterHandlerMap.get(routePath)(ctx);
    }

    return import(routePath).then((handle) => {
        const handler = handle.default;
        RouterHandlerMap.set(routePath, handler);
        return handler(ctx);
    });
};
