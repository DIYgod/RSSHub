...
// 遍历整个文件夹，过滤掉指定文件夹，并获取指定文件的绝对路径
function readDir(dirfather, filter = [], getFiles = []) {
    fs.readdir(dirfather, function(err, data) {
        if (err) {
            // console.error('停止迭代: ', err);
            return false;
        }
        // 这个data数组中装的是当前文件夹下所有的文件名(包括文件夹)
        data.forEach(function(el) {
            const srcPath = `${dirfather}/${el}`;
            // 判断是否文件或文件夹是否位于过滤列表，在过滤列表中则跳过。为空则不过滤任何文件或文件夹
            let filterFlag = false;
            if (filter.length !== 0) {
                filterFlag = filter.some((item) => item === el);
            }
            if (!filterFlag) {
                // 加上父级访问更深层的文件夹
                fs.stat(srcPath, function(errFiletype, dataFile) {
                    // 如果触发这个就证明程序错误
                    if (errFiletype) {
                        // console.error('读取文件错误: ', errFiletype);
                        return false;
                    }
                    // 又是文件夹
                    if (dataFile.isDirectory()) {
                        ...
                        // 迭代
                        readDir(srcPath, filter, getFiles);
                    } else {
                        // 只获取指定文件。为空则获取所有文件
                        let getFlag = true;
                        if (getFiles.length !== 0) {
                            getFlag = getFiles.some((item) => item === el);
                        }
                        if (getFlag) {
                            // 遍历文件
                            // console.log('是文件: ' + srcPath);
                            // 遍历所有文件夹，并自动导入所有插件的 router.js 文件，并加载路由
                            const RouterPath = require(srcPath);
                            for (const element in RouterPath) {
                                router.get(RouterPath[element].routerPath, require(`${dirfather}/` + RouterPath[element].indexFile));
                            }
                        }
                    }
                });
...
// 使用
const srcPath = __dirname + '/routes';
// 遍历整个 routes 文件夹，获取各个插件独立的 router.js 文件
readDir(srcPath, [], ['router.js', 'router-custom.js']);
