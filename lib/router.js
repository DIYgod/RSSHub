...
for (const dir in RouterPath) {
    const project = RouterPath[dir]['config.js']; // Do not merge other file
    if (project.routes) {
        project.routes.forEach((e) => {
            const modulePath = path.join(dirname, dir, e.module);
            router.get(e.path, lazyloadRouteHandler(modulePath)); // Support get for now
        });
    }
}
