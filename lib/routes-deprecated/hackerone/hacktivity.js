    ...
    ctx.state.data = {
        ...
        item: response.data.data.hacktivity_items.edges
            ...
            .map((item) => {
                ...
                const state = item.node.report.substate.replaceAll(/^[a-z]/g, (L) => L.toUpperCase());
                const severity = item.node.severity_rating ? item.node.severity_rating.replaceAll(/^[a-z]/g, (L) => L.toUpperCase()) : 'No Rating';
                const awarded = item.node.total_awarded_amount ?? 0;
                return {
