...
module.exports = async (ctx) => {
    ...
    description =
        Object.keys(data)[0] === 'online'
            ? `Status: ${Object.keys(data)[0]} <br> Guests: ${status.guests} <br>Registered: ${status.registered} <br> Other: ${status.other}  <br>Total: ${status.total} <br> Fa Server Time: ${data.fa_server_time}`
            : 'offline';
    const item = [];
    item.push({
