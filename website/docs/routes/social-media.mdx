...
<Route author="nightmare-mio" example="/bilibili/platform/-1/全部类型" path="/platform/:area?/:p_type?/:uid?" paramsDesc={['省市-国标码,默认->-1即全国','类型：见下表，默认->全部类型','用户id，可以不填，不过不填不设置cookie，搜索结果与登入账号后搜索结果不一样。可以在url中找到，需要配置cookie值，只需要SESSDATA的值即可']}/>
