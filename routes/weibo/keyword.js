...
module.exports = async (ctx) => {
    ...
    const response = await axios({
        method: 'get',
        url: `https://m.weibo.cn/api/container/getIndex?containerid=100103type%3D61%26q%3D${encodeURIComponent(
            keyword
        )}%26t%3D0`,
        headers: {
    ...
    ctx.state.data = {
        ...
        item: data.map((item) => {
            ...
            return {
                ...
                }: ${weiboUtils.format(item.mblog)}`,
                pubDate: weiboUtils.getTime(item.mblog.created_at),
                link: `https://weibo.com/${item.mblog.user.id}/${
                    item.mblog.bid
                }`,
            };
        }),
