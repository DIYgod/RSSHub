...
module.exports = async (ctx) => {
    ...
    ctx.state.data = {
        ...
        item: data.map((item) => {
            ...
            const linkMap = {
                OFFICIAL_MESSAGE: `https://web.okjike.com/message-detail/${
                    item.id
                }/officialMessage`,
                ORIGINAL_POST: `https://web.okjike.com/post-detail/${
                    item.id
                }/originalPost`,
            ...
            if (item.linkInfo && item.linkInfo.linkUrl) {
                linkTemplate = `<a href="${item.linkInfo.linkUrl}">${
                    item.linkInfo.title
                }</a><br>`;
            }
            ...
            item.pictures &&
                item.pictures.forEach((item) => {
                    imgTemplate += `<img referrerpolicy="no-referrer" src="${
                        item.picUrl
                    }"><br>`;
                });
            ...
            if (item.type === 'REPOST') {
                const screenNameTemplate = item.target.user
                    ? `<a href="https://web.okjike.com/user/${
                ...
                item.target.pictures &&
                    item.target.pictures.forEach((item) => {
                        repostImgTemplate += `<br><img referrerpolicy="no-referrer" src="${
                            item.thumbnailUrl
                        }">`;
                    });

                const repostContent = `转发 ${screenNameTemplate}: ${
                    item.target.content
                }${repostImgTemplate}`;
            ...
            if (item.type === 'ANSWER') {
                ...
                item.richtextContent.blocks &&
                    item.richtextContent.blocks.forEach((item) => {
                        if (
                            item.entityRanges.length &&
                            item.text === '[图片]'
                        ) {
                            answerImgKeys = [
                                ...answerImgKeys,
                                ...Object.keys(item.entityRanges),
                            ];
                        } else {
                ...
                if (answerImgKeys.length) {
                    answerImgKeys.forEach((key) => {
                        ...
                        if (entity.type.toUpperCase() === 'IMAGE') {
                            answerImgTemplate += `<br><img referrerpolicy="no-referrer" src="${
                                entity.data.pictureUrl.middlePicUrl
                            }">`;
                        }
                    });
                }
