...
module.exports = async (ctx) => {
    ...
    const location = `${section}${ctx.params.id}${typefilter}&orderby=dateline&ascdesc=DESC`;
    let title = '';
    ...
    const fetch = async (pageindex) => {
        ...
        const res = await axios_ins.get(pageUrl);
        ...
        const result = await Promise.all(
            list.map(async (item) => {
                ...
                let title = $('span a');
                const catalog = $('em').text();
                const path = title.attr('href');
                title = title.text();
                const link = url.resolve(base, `/bbs/${path}`);
