...
<Route author="lengthmin jjeejj prnake" example="/ximalaya/album/299146/0/1" path="/ximalaya/album/:id/:all/:shownote?" :paramsDesc="['专辑 id, 可在对应专辑页面的 URL 中找到','是否需要获取全部节目，填入 `1`、`true`、`all` 视为获取所有节目，填入其他则不获取。', '是否需要获取节目的 ShowNote，填入 `1`、`true` 视为获取']" supportPodcast="1" selfhost="1">
