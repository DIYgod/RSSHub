...
<Route author="DIYgod" example="/twitter/user/DIYgod" path="/twitter/user/:id/:type?" :paramsDesc="['用户名', '额外选项 `include_replies`包含回复，`include_rts`包含转推，`include_rts_replies`全部包含，默认 `include_replies`包含回复']" radar="1"/>
