...
<RouteEn author="nczitzk" example="/fashionnetwork/news/5,6/15,112" path="/fashionnetwork/news/:sectors?/:categories?/:language?" :paramsDesc="['Sectors, see below, `all` by default', 'Categories, see below, `all` by default', 'Language, see below, `ww` as Worldwide by default']">
